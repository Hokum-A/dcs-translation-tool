...
class TTSWorker(QThread):
    ...
    def run(self):
        try:
            ...
            if self.kwargs.get('convert_to_ogg', False):
                ogg_path = os.path.splitext(self.output_path)[0] + ".ogg"
                if os.path.exists(ogg_path):
                    final_path = ogg_path
                    
            if success and os.path.exists(final_path) and os.path.getsize(final_path) > 44:
...
class TTSEngine:
    ...
    def install_tts_libraries(self, progress_callback=None, text_callback=None):
        ...
        try:
            ...
            # Патч TTS/__init__.py: отключаем проверку torchcodec, которая
            # крашится на Windows без FFmpeg. Вместо torchcodec мы используем
            # soundfile через monkey-patch в xtts_server.py.
            try:
                python_dir = Path(python_exe).parent
                tts_init = python_dir / "Lib" / "site-packages" / "TTS" / "__init__.py"
                if tts_init.exists():
                    ...
                    patched = content.replace(
                        ...
                        '# [PATCHED] torchcodec check disabled — using soundfile via xtts_server.py\n# if is_torch_greater_or_equal("2.9"):\n#     if not is_torchcodec_available():\n#         raise ImportError(TORCHCODEC_IMPORT_ERROR)'
                    )
                    if patched != content:
                        tts_init.write_text(patched, encoding="utf-8")
                        log("Patched TTS/__init__.py: disabled torchcodec requirement check")
                    else:
                        log("TTS/__init__.py: torchcodec check already patched or not found")
            except Exception as e:
                log(f"Warning: could not patch TTS/__init__.py: {e}")
    ...
    def generate_speech(self, text, output_path, lang='ru', engine='piper', voice_id=None, convert_to_ogg=False, **kwargs):
        ...
        args:
            ...
            convert_to_ogg: Если True, конвертирует итоговый .wav файл в .ogg
        kwargs:
        ...
        if engine == 'xtts':
            if self.is_xtts_available():
                success = self._generate_xtts(text, output_path, lang, voice_id=voice_id, **kwargs)
            else:
                logger.warning("XTTS v2 not installed, falling back to Piper")
                engine = 'piper'

        # Логика Piper
        if not success and engine == 'piper' and self._check_piper_available():
            ...
            if voice_id and self._get_model_path(voice_id).exists():
                success = self._generate_piper(text, output_path, voice_id, **kwargs)

        # Фолбэк на системный движок
        if not success:
            success = self._generate_system(text, output_path, lang)

        if success and convert_to_ogg and os.path.exists(output_path):
            ogg_path = self.convert_wav_to_ogg(output_path)
            if ogg_path:
                return True
    ...
    def ensure_ffmpeg(self):
        """Скачивает ffmpeg.exe (необходимо для конвертации в OGG)."""
        ffmpeg_exe = self.base_dir / "ffmpeg.exe"
        if ffmpeg_exe.exists():
            return True
            
        logger.info("Downloading ffmpeg for OGG conversion...")
        try:
            import urllib.request
            import zipfile
            import io
            url = "https://github.com/ffbinaries/ffbinaries-prebuilt/releases/download/v4.4.1/ffmpeg-4.4.1-win-64.zip"
            # Для упрощения не используем прогресс-бар, т.к. это выполняется в фоновом потоке
    ...
    def convert_wav_to_ogg(self, wav_path):
        """Конвертирует WAV в OGG. Удаляет исходный WAV. Возвращает путь к OGG."""
        ...
        try:
            ...
            subprocess.run([
                ...
                '-c:a', 'libvorbis', '-q:a', '4',
                str(ogg_path)
            ], capture_output=True, check=True, creationflags=subprocess.CREATE_NO_WINDOW if os.name == 'nt' else 0)
            
            if os.path.exists(ogg_path) and os.path.getsize(ogg_path) > 100:
                os.remove(wav_path)
                return ogg_path
        except Exception as e:
            logger.error(f"Error converting {wav_path} to OGG: {e}")
    ...
    def _generate_xtts(self, text, output_path, lang='ru', voice_id=None, **kwargs):
        ...
        if not self._init_xtts():
            raise Exception("Failed to initialize or start XTTS server.")
        ...
        try:
            ...
            if result.get("status") == "ok":
                ...
                raise Exception(f"XTTS Error: {error_msg}")
    ...
    def _download_file(self, url, dest, label, external_callback=None, text_callback=None, ip_callback=None):
        ...
        try:
            ...
            # Для файлов >20 МБ с поддержкой Range — многопоточная загрузка (8 потоков)
            MIN_SIZE_FOR_THREADS = 20 * 1024 * 1024
            NUM_THREADS = 8
            if total_size > MIN_SIZE_FOR_THREADS and 'bytes' in accept_ranges.lower():
                logger.info(f"{label}: попытка многопоточной загрузки ({NUM_THREADS} потоков, {total_size} байт)")
                if self._download_file_threaded(url, dest, label, NUM_THREADS, total_size, ctx, headers, external_callback, text_callback):
                    return True
                    
                logger.warning(f"{label}: многопоточная загрузка не удалась, fallback на однопоточную...")
