...
from PyQt5.QtWidgets import (QApplication, QMainWindow, QWidget, QVBoxLayout,
                             ...
                             QSizePolicy, QDialog, QToolTip, QGridLayout, QComboBox, QProgressBar, QTextBrowser, QShortcut, QToolButton, QWidgetAction)
...
from dialogs import (CustomDialog, MizFolderDialog,
                    ...
                    TTSPreviewDialog, TTSLoadingDialog, StandardInfoDialog)
...
"""
        ...
        self.smart_paste_enabled = True  # Умная вставка с маркерами по умолчанию ВКЛ
        self.recent_files = []
        ...
        self._last_preview_enabled_state = True
        ...
        self.bookmark_bg_star = '#423400'
        self.bookmark_bg_question = '#0d4200'
        self.bookmark_bg_alert = '#420000'
        self.bookmark_bg_opacity = 0.60
    ...
    def init_ui(self):
        ...
        # [SMART PASTE] Перехват Ctrl+V для вставки маркеров 🔹
        self.translated_text_all.installEventFilter(self)
    ...
    def setup_ui_components(self, main_layout):
        ...
        self.main_vertical_splitter.splitterMoved.connect(self.on_main_splitter_moved)

    def on_main_splitter_moved(self, pos, index):
        """Обработка ручного схлопывания панели предпросмотра"""
        sizes = self.main_vertical_splitter.sizes()
        # Если высота превью (второй элемент) стала равна 0
        if sizes and len(sizes) > 1 and sizes[1] == 0:
            self.preview_collapsed = True
            if hasattr(self, 'preview_batch_timer'):
                self.preview_batch_timer.stop()
        else:
            was_collapsed = getattr(self, 'preview_collapsed', False)
            self.preview_collapsed = False
            # Если развернули обратно и включено в настройках
            if was_collapsed and getattr(self, 'preview_enabled', True):
                self.update_preview()

    def apply_preview_enabled_setting(self):
        """Применяет изменение настройки включения/выключения предпросмотра"""
        is_enabled = getattr(self, 'preview_enabled', True)
        
        # Если состояние не изменилось, ничего не делаем, чтобы не вызывать лишнюю перерисовку
        if getattr(self, '_last_preview_enabled_state', None) == is_enabled:
            return
            
        self._last_preview_enabled_state = is_enabled
        
        if not is_enabled:
            # Скрываем
            sizes = self.main_vertical_splitter.sizes()
            if sizes and sizes[1] > 0:
                self.last_preview_sizes = sizes
                self.main_vertical_splitter.setSizes([1000, 0])
            ...
            sizes = self.main_vertical_splitter.sizes()
            if sizes and sizes[1] == 0:
                self.main_vertical_splitter.setSizes(getattr(self, 'last_preview_sizes', [500, 500]))
            self.preview_collapsed = False
            self.update_preview()
    ...
    def find_mission_name_key_index(self):
        ...
        Для .miz: DictKey_sortie_..., name_RU, name_DE и т.д.
    ...
    def setup_top_panel(self, main_layout):
        ...
        # Кнопка перевода Lua скриптов (скрыта по умолчанию)
        self.lua_scripts_btn = QPushButton(get_translation(self.current_language, 'lua_btn_text'))
        self.lua_scripts_btn.setFixedSize(220, 24)
        self.lua_scripts_btn.setCursor(Qt.PointingHandCursor)
        self.lua_scripts_btn.clicked.connect(self.open_lua_translation_window)
        self.register_custom_tooltip(self.lua_scripts_btn, get_translation(self.current_language, 'lua_btn_tooltip'), side='bottom')
        self.lua_scripts_btn.setStyleSheet("""
            QPushButton {
        ...
        """)
        self.lua_scripts_btn.setVisible(False)
        selected_miz_inner_layout.addWidget(self.lua_scripts_btn, 0, Qt.AlignVCenter)
        ...
        right_buttons_layout = QGridLayout(right_buttons_container)
        right_buttons_layout.setContentsMargins(0, 0, 0, 0)
        right_buttons_layout.setSpacing(5) # Отступ между кнопками по вертикали и горизонтали
        ...
        right_buttons_layout.addWidget(self.instructions_btn, 0, 1, Qt.AlignRight)
        
        self.ai_lang_flag_label = QLabel()
        self.ai_lang_flag_label.setStyleSheet("font-size: 13px; font-weight: bold; color: #aaaaaa; background-color: transparent;")
        self.update_ai_flag_label()
        
        right_buttons_layout.addWidget(self.ai_lang_flag_label, 1, 0, Qt.AlignRight | Qt.AlignVCenter)
        right_buttons_layout.addWidget(self.ai_context_mgmt_btn, 1, 1, Qt.AlignRight)
    ...
    def eventFilter(self, obj, event):
        ...
        # [SMART PASTE] Перехват Ctrl+V в поле перевода для вставки с маркерами 🔹
        # Не обрабатываем если открыто модальное окно (например, окно перевода Lua)
        if (hasattr(self, 'translated_text_all') and obj is self.translated_text_all 
                and event.type() == QEvent.KeyPress 
                and event.key() == Qt.Key_V and event.modifiers() & Qt.ControlModifier
                and QApplication.activeModalWidget() is None):
            clipboard_text = QApplication.clipboard().text()
            import re
            if clipboard_text and re.search(r'🔹\d+🔹', clipboard_text) and getattr(self, 'smart_paste_enabled', True):
                self.paste_from_clipboard()
                return True  # Блокируем стандартную вставку Qt
    ...
    def show_instructions(self):
        ...
        try:
            if not hasattr(self, 'instructions_window') or not self.instructions_window.isVisible():
    ...
    def update_ai_flag_label(self):
        if not hasattr(self, 'ai_lang_flag_label'):
            return
            
        lang = getattr(self, 'ai_context_lang_1', 'RU')
        flag_path = resource_path(f"images/{lang.upper()}.png")
        
        if os.path.exists(flag_path):
            self.ai_lang_flag_label.setText(f'<img src="{flag_path}" width="24" height="18"> {lang}')
        else:
            self.ai_lang_flag_label.setText(f"{lang}")
            
        if hasattr(self, 'ai_context_mgmt_btn'):
            base_text = get_translation(self.current_language, 'ai_context_mgmt_btn')
            self.ai_context_mgmt_btn.setText(base_text)
    ...
    def save_ai_context_settings(self, context_1, context_2, lang_1=None):
        ...
        if lang_1:
            ...
            if hasattr(self, 'update_ai_flag_label'):
                self.update_ai_flag_label()
    ...
    def setup_translation_area(self, parent_container):
        ...
        # Состояние для частичного копирования
        self._chunk_count = 2
        self._copied_parts = set()
        self._last_copied_part = None
        
        self.copy_all_btn = QToolButton()
        self.copy_all_btn.setText('📋 Копировать весь текст')
        self.copy_all_btn.setPopupMode(QToolButton.MenuButtonPopup)
        ...
        # Выпадающее меню для копирования частями
        self.copy_menu = QMenu(self)
        self.copy_menu.setStyleSheet("""
            QMenu {
                background-color: #2b2b2b;
                color: #ffffff;
                border: 1px solid #ff9900;
        ...
        """)
        self.copy_all_btn.setMenu(self.copy_menu)
        self.copy_menu.aboutToShow.connect(self._build_copy_menu)
        ...
        add_context_layout.setContentsMargins(0, 0, 0, 0)
        ...
        # Блок "Оптимизация текста"
        self.optimize_text_container = QWidget()
        self.optimize_text_container.setStyleSheet("background-color: transparent; border: none;")
        optimize_text_layout = QHBoxLayout(self.optimize_text_container)
        optimize_text_layout.setContentsMargins(0, 0, 0, 0)
        optimize_text_layout.setSpacing(6)
        
        self.optimize_text_toggle = ToggleSwitch()
        self.optimize_text_toggle.setChecked(getattr(self, 'optimize_text', True))
        self.optimize_text_toggle.toggled.connect(self.on_optimize_text_toggled)
        
        self.optimize_text_label_widget = QLabel(get_translation(self.current_language, 'optimize_text_label'))
        self.optimize_text_label_widget.setStyleSheet("color: #ddd; background-color: transparent; border: none; font-size: 12px;")
        
        optimize_text_layout.addWidget(self.optimize_text_toggle)
        optimize_text_layout.addWidget(self.optimize_text_label_widget)
        
        self.register_custom_tooltip(self.optimize_text_label_widget, get_translation(self.current_language, 'tooltip_optimize_text'))

        # Стрелочки цепочки действий
        self._chain_arrow1 = QLabel()
        self._chain_arrow2 = QLabel()
        
        # Функции обновления стрелочек
        def update_arrow1(checked):
            img_name = "Left_arrow_orange.png" if checked else "Left_arrow_gray.png"
            self._chain_arrow1.setPixmap(QPixmap(resource_path(f"images/{img_name}")))
            
        def update_arrow2(checked):
            img_name = "Left_arrow_orange.png" if checked else "Left_arrow_gray.png"
            self._chain_arrow2.setPixmap(QPixmap(resource_path(f"images/{img_name}")))
            
        # Подключаем сигналы
        self.add_context_toggle.toggled.connect(update_arrow1)
        self.optimize_text_toggle.toggled.connect(update_arrow2)
        
        # Задаем начальное состояние
        update_arrow1(self.add_context_toggle.isChecked())
        update_arrow2(self.optimize_text_toggle.isChecked())
        
        # Вертикальная компоновка для тумблеров
        toggles_container = QWidget()
        toggles_layout = QVBoxLayout(toggles_container)
        toggles_layout.setContentsMargins(10, 0, 0, 0)
        ...
        row1_layout.addWidget(self._chain_arrow1)
        row1_layout.addWidget(self.add_context_container)
        ...
        row2_layout.addWidget(self._chain_arrow2)
        row2_layout.addWidget(self.optimize_text_container)
        row2_layout.addStretch()
        
        toggles_layout.addWidget(row1)
        toggles_layout.addWidget(row2)
        toggles_layout.addStretch() # Чтобы прижать наверх, если потребуется
        
        left_buttons.addWidget(self.copy_all_btn, 0, Qt.AlignVCenter)
        left_buttons.addWidget(toggles_container, 0, Qt.AlignVCenter)
        left_buttons.addStretch()
        left_buttons.addWidget(self.show_keys_btn, 0, Qt.AlignVCenter)
    ...
    def setup_preview_panel(self, parent_container):
        ...
        self.preview_header_ref_combo.setFixedWidth(95)  # Увеличено с 85, чтобы помещалось слово DEFAULT
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.settings_file):
            try:
                ...
                self.smart_paste_enabled = settings.get('smart_paste_enabled', True)
                ...
                self.preview_enabled = settings.get('preview_enabled', True)
                self._last_preview_enabled_state = self.preview_enabled
                self.last_preview_sizes = settings.get('last_preview_sizes', [500, 500])
                self.preview_collapsed = False
                ...
                # Загружаем настройки контекста и оптимизации
                self.add_context = settings.get('add_context', True)
                self.optimize_text = settings.get('optimize_text', True)
                self.lua_optimize_text = settings.get('lua_optimize_text', True)
                self.lua_add_context = settings.get('lua_add_context', True)
                if hasattr(self, 'add_context_toggle'):
                    self.add_context_toggle.setChecked(self.add_context)
                if hasattr(self, 'optimize_text_toggle'):
                    self.optimize_text_toggle.setChecked(self.optimize_text)
                ...
                if 'main_vertical_splitter_sizes' in settings and hasattr(self, 'main_vertical_splitter'):
                    ...
                    if getattr(self, 'preview_enabled', True):
                        # Защита от полного скрытия окна, если предпросмотр ВКЛЮЧЕН
                        if len(sizes) == 2 and sizes[1] < 100:
                            sizes = getattr(self, 'last_preview_sizes', [500, 500])
                            if sizes[1] < 100:
                                sizes = [max(100, sizes[0] - 100), max(100, sizes[0] // 3)]
                    else:
                        # Если предпросмотр выключен, принудительно схлопываем
                ...
                self.saved_lua_window_geom = settings.get('lua_window_geom')
                ...
                self.bookmark_bg_star = settings.get('bookmark_bg_star', '#423400')
                self.bookmark_bg_question = settings.get('bookmark_bg_question', '#0d4200')
                self.bookmark_bg_alert = settings.get('bookmark_bg_alert', '#420000')
                self.bookmark_bg_opacity = settings.get('bookmark_bg_opacity', 0.60)
    ...
    def save_settings(self, force=False, update_preview=True, update_ui=True):
        ...
        try:
            ...
            # Автоматическое отключение тумблера оптимизации, если умная вставка выключена
            if not getattr(self, 'smart_paste_enabled', True) and getattr(self, 'optimize_text', False):
                if hasattr(self, 'optimize_text_toggle'):
                    # Вызываем setChecked без блокировки сигналов, чтобы стрелочка и UI обновились
                    self.optimize_text_toggle.setChecked(False)
                self.optimize_text = False
            ...
            settings = {
                ...
                'optimize_text': self.optimize_text_toggle.isChecked() if hasattr(self, 'optimize_text_toggle') else getattr(self, 'optimize_text', True),
                'lua_optimize_text': getattr(self, 'lua_optimize_text', True),
                'lua_add_context': getattr(self, 'lua_add_context', True),
                ...
                'lua_window_geom': getattr(self, 'saved_lua_window_geom', None),
                ...
                'bookmark_bg_star': getattr(self, 'bookmark_bg_star', '#423400'),
                'bookmark_bg_question': getattr(self, 'bookmark_bg_question', '#0d4200'),
                'bookmark_bg_alert': getattr(self, 'bookmark_bg_alert', '#420000'),
                'bookmark_bg_opacity': getattr(self, 'bookmark_bg_opacity', 0.60),
                ...
                'smart_paste_enabled': getattr(self, 'smart_paste_enabled', True),
                'preview_enabled': getattr(self, 'preview_enabled', True),
                'last_preview_sizes': getattr(self, 'last_preview_sizes', [500, 500]),
                'custom_filters': []
            }
    ...
    def update_interface_language(self):
        ...
        if hasattr(self, 'instructions_window') and self.instructions_window:
            self.instructions_window.update_language(self.current_language)
        ...
        # Кнопка перевода Lua скриптов
        if hasattr(self, 'lua_scripts_btn'):
            self.lua_scripts_btn.setText(get_translation(self.current_language, 'lua_btn_text'))
            self.register_custom_tooltip(self.lua_scripts_btn, get_translation(self.current_language, 'lua_btn_tooltip'), side='bottom')
        ...
        # Обновляем элементы нижней панели
        ...
        if hasattr(self, 'optimize_text_label_widget'):
            self.optimize_text_label_widget.setText(get_translation(self.current_language, 'optimize_text_label'))
            self.register_custom_tooltip(self.optimize_text_label_widget, get_translation(self.current_language, 'tooltip_optimize_text'))
    ...
    def update_translation_stats(self):
        ...
        if not self.original_lines:
            self.stats_label.setText(get_translation(self.current_language, 'stats_lines', count=0))
            return
    ...
    def parse_dictionary_file(self, content, progress=None):
        ...
        try:
            ...
            if progress: progress.set_value(75)
            ...
            line_number = 0
            for i, (key, (text_parts, file_lines, absolute_start_line)) in enumerate(entries.items()):
                if progress and total_items > 0 and i % max(1, total_items // 10) == 0:
                    prog_val = 75 + int((i / total_items) * 10)
                    progress.set_value(prog_val)
    ...
    def open_lua_translation_window(self):
        """Открывает автономное окно перевода Lua скриптов"""
        # Проверяем несохранённые изменения
        if getattr(self, 'has_unsaved_changes', False):
            self.show_custom_dialog(
                get_translation(self.current_language, 'lua_save_first_title'),
                get_translation(self.current_language, 'lua_save_first'),
                "info"
            )
            return
        ...
        from dialogs import LuaTranslationWindow
        
        # Передаём AI контекст из основной программы
        ai_context = getattr(self, 'ai_context_1', '')
        context_on = self.add_context_toggle.isChecked() if hasattr(self, 'add_context_toggle') else False
        
        win = LuaTranslationWindow(
            self.current_miz_path,
        ...
        if hasattr(self, 'saved_lua_window_geom') and self.saved_lua_window_geom:
            try:
                win.setGeometry(*self.saved_lua_window_geom)
            except Exception:
                pass
        ...
        try:
            self.saved_lua_window_geom = [win.x(), win.y(), win.width(), win.height()]
            self.save_settings(update_preview=False, update_ui=False)
        except Exception:
            pass
    ...
    def open_miz_file(self, preselected_path=None):
        ...
        try:
            ...
            self.parse_dictionary_file(self.original_content, progress=progress)
            
            if self.all_lines_data:
                self.apply_filters(progress=progress)
                self.save_file_btn.setEnabled(True)
                ...
                # Быстрая проверка наличия Lua скриптов с переводимым текстом
                try:
                    from lua_script_parser import LuaScriptParser
                    lua_parser = LuaScriptParser()
                    has_lua = lua_parser.quick_scan(file_path)
                    if hasattr(self, 'lua_scripts_btn'):
                        self.lua_scripts_btn.setVisible(has_lua)
                    if has_lua:
                        print(f"DEBUG: Lua scripts with translatable text detected in {os.path.basename(file_path)}")
                except Exception as e:
                    print(f"WARNING: Lua scan failed: {e}")
    ...
    def apply_filters(self, full_rebuild=True, progress=None):
        """Применяет выбранные фильтры к данным"""
        lines_before = len(self.original_lines)
        old_line_ids = [id(x) for x in self.original_lines]
        self.original_lines = []
        
        # Сбрасываем карту дедубликации, так как она будет перестроена при копировании
        self.translation_dedup_map = {}
        ...
        total_lines = len(self.all_lines_data)
        for idx, line_data in enumerate(self.all_lines_data):
            if progress and total_lines > 0 and idx % max(1, total_lines // 15) == 0:
                prog_val = 85 + int((idx / total_lines) * 15)
                progress.set_value(prog_val)
        ...
        # Если состав или порядок визуальных строк изменился, сбрасываем прогресс копирования частей
        new_line_ids = [id(x) for x in self.original_lines]
        if old_line_ids != new_line_ids:
            if hasattr(self, '_copied_parts'):
    ...
    def update_display(self, update_preview=True):
        ...
        try:
            ...
            if update_preview and not getattr(self, 'is_updating_from_preview', False):
                ...
                # Обновляем цвета темы в превью без перерисовки виджетов
                if hasattr(self, 'update_preview_theme_colors'):
    ...
    def on_optimize_text_toggled(self, checked):
        """Handler for `optimize_text_toggle`: save setting."""
        if checked and not getattr(self, 'smart_paste_enabled', True):
            # Откладываем отключение на 150мс, чтобы тумблер успел дернуться вправо (анимация отскока)
            from PyQt5.QtCore import QTimer, QRect, QPoint
            QTimer.singleShot(150, lambda: self.optimize_text_toggle.setChecked(False))
            
            # Показываем красивый кастомный тултип над тумблером
            from widgets import CustomToolTip
            from PyQt5.QtCore import QTimer, QRect, QPoint
            
            # Если старый тултип еще висит - просто скрываем
            try:
                if hasattr(self, '_optimize_error_tooltip') and self._optimize_error_tooltip:
                    self._optimize_error_tooltip.hide()
            except RuntimeError:
                pass
            
            msg = get_translation(self.current_language, 'error_optimize_requires_smart_paste')
            popup = CustomToolTip(self)
            self._optimize_error_tooltip = popup
            
            global_pos = self.optimize_text_toggle.mapToGlobal(QPoint(0, 0))
            global_rect = QRect(global_pos, self.optimize_text_toggle.size())
            popup.show_tooltip_at_rect(msg, global_rect, 'bottom')
            
            # Автоматическое скрытие через 3.5 секунды (без deleteLater, чтобы избежать крашей при быстрых кликах)
            def hide_tooltip():
                try:
                    popup.hide()
                except RuntimeError:
                    pass
            QTimer.singleShot(3500, hide_tooltip)
            return
            
        try:
            self.optimize_text = bool(checked)
            self.save_settings(update_preview=False, update_ui=False)
        except Exception:
            try:
                self.save_settings(update_ui=False)
    ...
    def copy_all_english(self):
        ...
        # Очищаем старую карту дедубликации
        self.translation_dedup_map = {}
        text_to_id = {}
        
        # Регулярка: строка должна содержать хотя бы одну букву (любой язык), чтобы считаться переводимой
        import re
        has_letters_re = re.compile(r'[a-zA-Zа-яА-Я]')
        # Регулярка: координаты (строка должна целиком состоять только из N/S, цифр, E/W и знаков препинания/пробелов)
        coords_re = re.compile(r'^[NSns]\s*\d+[0-9°\'".,\s\-]*[EWew]\s*\d+[0-9°\'".,\s\-]*$')
        
        optimize_enabled = getattr(self, 'optimize_text', True)
        if hasattr(self, 'optimize_text_toggle'):
            optimize_enabled = self.optimize_text_toggle.isChecked()
            
        skipped_count = 0
        total_count = len(self.original_lines)
        ...
        for i, line_data in enumerate(self.original_lines):
            ...
            if getattr(self, 'smart_paste_enabled', True):
                if optimize_enabled:
                    stripped = clean_text.strip()
                    # Пропускаем пустые и бессмысленные строки (только цифры/знаки)
                    if not stripped or not has_letters_re.search(stripped):
                        skipped_count += 1
                        continue
                        
                    # Пропускаем одиночные символы ("Z", "P", "L" и т.д.)
                    if len(stripped) <= 1:
                        skipped_count += 1
                        continue
                        
                    # Пропускаем технические координаты (N 33°... E 35°...)
                    ...
                    if clean_text in text_to_id:
                        primary_idx = text_to_id[clean_text]
                        self.translation_dedup_map[primary_idx].append(idx)
                        skipped_count += 1
                    else:
                        text_to_id[clean_text] = idx
                        self.translation_dedup_map[idx] = [idx]
                        english_lines.append(f"🔹{idx}🔹 {clean_text}")
                else:
                    english_lines.append(f"🔹{idx}🔹 {clean_text}")
            else:
                english_lines.append(clean_text)
        ...
        if backslash_lines:
            ...
            msg = f"✅ Скопировано {len(english_lines)} строк. ⚠ Строки со слешом: {', '.join(map(str, shown))}{tail}"
            self.statusBar().showMessage(msg)
        else:
            if optimize_enabled and getattr(self, 'smart_paste_enabled', True):
                ratio = int((skipped_count / total_count) * 100) if total_count > 0 else 0
                msg = get_translation(self.current_language, 'status_optimized_copy', total=total_count, unique=len(english_lines), ratio=ratio)
                self.statusBar().showMessage(msg)
            else:
                self.statusBar().showMessage(get_translation(self.current_language, 'status_copied', count=len(english_lines)))
    
    # ──────────────────────────────────────────────
    #  Меню копирования частями (главное окно)
    # ──────────────────────────────────────────────

    def _build_copy_menu(self):
        """Динамически строит выпадающее меню перед каждым показом"""
        lang = self.current_language
        self.copy_menu.clear()

        # Если умная вставка отключена, копирование по частям невозможно
        if not getattr(self, 'smart_paste_enabled', True):
            act = self.copy_menu.addAction(get_translation(lang, 'lua_chunk_disabled_smart_paste'))
            act.setEnabled(False)
            return

        # --- Кастомный виджет: «Разбить на: [◄] N [►]» ---
        header_widget = QWidget()
        header_layout = QHBoxLayout(header_widget)
        header_layout.setContentsMargins(8, 6, 8, 6)
        header_layout.setSpacing(6)

        lbl = QLabel(get_translation(lang, 'lua_chunk_split_label'))
        ...
        btn_dec.setStyleSheet("""
            QPushButton { background: #444; color: #fff; border: none; border-radius: 4px; font-size: 13px; font-weight: bold; }
            QPushButton:hover { background: #ff9900; color: #000; }
        """)
        btn_dec.clicked.connect(lambda: self._change_chunk_count(-1))
        header_layout.addWidget(btn_dec)

        self._chunk_count_label = QLabel(str(self._chunk_count))
        self._chunk_count_label.setFixedWidth(24)
        self._chunk_count_label.setAlignment(Qt.AlignCenter)
        self._chunk_count_label.setStyleSheet("color: #ff9900; font-size: 14px; font-weight: bold; background: transparent;")
        header_layout.addWidget(self._chunk_count_label)
        ...
        btn_inc.setStyleSheet("""
            ...
            QPushButton:hover { background: #ff9900; color: #000; }
        """)
        btn_inc.clicked.connect(lambda: self._change_chunk_count(+1))
        header_layout.addWidget(btn_inc)

        header_layout.addStretch()

        header_action = QWidgetAction(self.copy_menu)
        header_action.setDefaultWidget(header_widget)
        self.copy_menu.addAction(header_action)
        self.copy_menu.addSeparator()

        # --- Пункты для каждой части ---
        total = len(self.original_lines)
        if total == 0:
            act = self.copy_menu.addAction(get_translation(lang, 'lua_chunk_no_data'))
            act.setEnabled(False)
            return

        chunk_size = max(1, (total + self._chunk_count - 1) // self._chunk_count)
        ...
        for i in range(self._chunk_count):
            ...
            if i == getattr(self, '_last_copied_part', None):
                prefix = "✅ "
            elif i in getattr(self, '_copied_parts', set()):
                prefix = "✓ "
            else:
                prefix = "   "

            text = get_translation(lang, 'lua_chunk_part',
                                   prefix=prefix, part=i + 1,
                                   start=start, end=end)
            act = self.copy_menu.addAction(text)
            part_idx = i
            act.triggered.connect(lambda checked, idx=part_idx: self._copy_part(idx))

    def _change_chunk_count(self, delta):
        """Изменяет количество частей на delta, перестраивает меню"""
        new_val = self._chunk_count + delta
        if new_val < 2:
            new_val = 2
        if new_val > 20:
            new_val = 20
        if new_val != self._chunk_count:
            self._chunk_count = new_val
            self._copied_parts.clear()
            self._last_copied_part = None
            self._chunk_count_label.setText(str(self._chunk_count))
            self.copy_menu.close()
            QTimer.singleShot(50, lambda: self.copy_all_btn.showMenu())

    def _copy_part(self, part_index):
        """Копирует конкретную часть текста (с учётом глобальной оптимизации)"""
        import re
        lang = self.current_language
        ...
        # 1. Сначала применяем глобальную оптимизацию ко всем строкам
        lines_to_chunk = []
        global_skipped = 0
        ...
        if optimize_enabled and getattr(self, 'smart_paste_enabled', True):
            ...
            for i, line_data in enumerate(self.original_lines):
                ...
                if clean_text in text_to_id:
                    ...
                    text_to_id[clean_text] = idx
                    self.translation_dedup_map[idx] = [idx]
                    lines_to_chunk.append(f"🔹{idx}🔹 {clean_text}")
        else:
            for i, line_data in enumerate(self.original_lines):
                idx = i + 1
                clean_text = line_data['display_text'].replace('\n', ' ')
                ...
                if getattr(self, 'smart_paste_enabled', True):
                    ...
                    lines_to_chunk.append(clean_text)

        total_optimized = len(lines_to_chunk)
        if total_optimized == 0:
            return

        # 2. Разбиваем уже оптимизированный список на части
        chunk_size = max(1, (total_optimized + self._chunk_count - 1) // self._chunk_count)
        start_idx = part_index * chunk_size
        end_idx = min((part_index + 1) * chunk_size, total_optimized)

        part_lines = lines_to_chunk[start_idx:end_idx]
        if not part_lines:
            return
        ...
        # Статус
        if optimize_enabled and getattr(self, 'smart_paste_enabled', True):
            ratio = int((global_skipped / total_count) * 100) if total_count > 0 else 0
            msg = get_translation(lang, 'status_optimized_part_copy',
                                  part=part_index + 1, parts=self._chunk_count,
                                  total=total_count, unique=len(part_lines),
                                  ratio=ratio)
            self.statusBar().showMessage(msg)
        else:
            self.statusBar().showMessage(
                get_translation(lang, 'status_part_copied',
                                part=part_index + 1, parts=self._chunk_count,
                                count=len(part_lines))
            )

        # Добавляем AI контекст, если включён
        is_context_enabled = self.add_context_toggle.isChecked() if hasattr(self, 'add_context_toggle') else getattr(self, 'add_context', True)
        if is_context_enabled and hasattr(self, 'ai_context_1') and self.ai_context_1.strip():
            result_text = self.ai_context_1.strip() + "\n\n" + result_text

        QApplication.clipboard().setText(result_text)

        # Отмечаем часть как скопированную и как последнюю
        self._copied_parts.add(part_index)
        self._last_copied_part = part_index

    def _build_dedup_map_if_needed(self):
        """Строит карту дедубликации по всем original_lines, если нужно"""
        import re
        ...
        if not optimize_enabled or not getattr(self, 'smart_paste_enabled', True):
            return

        if getattr(self, 'translation_dedup_map', None):
            return
    ...
    def paste_from_clipboard(self):
        ...
        # Не вставляем если открыто модальное окно (например, LuaTranslationWindow)
    ...
    def handle_smart_paste_request(self, clipboard_text):
        """
        [SMART PASTE] Обрабатывает запрос на умную вставку из дочерних виджетов (превью).
        Передаёт текст с маркерами в основное текстовое поле, что автоматически
        вызывает on_translation_changed и распределяет строки по всем виджетам.
        """
        # Не вставляем если открыто модальное окно
        if QApplication.activeModalWidget() is not None:
            return
        if clipboard_text:
            self.translated_text_all.setPlainText(clipboard_text)
            self.statusBar().showMessage(get_translation(self.current_language, 'status_text_pasted'))
    ...
    def clear_current_data(self):
        ...
        if hasattr(self, '_copied_parts'):
            self._copied_parts.clear()
        if hasattr(self, '_last_copied_part'):
            self._last_copied_part = None
    ...
    def _get_bookmark_visual(self, key):
        ...
        if bm:
            ...
            if b_type == 'star':
                bg_color_hex = getattr(self, 'bookmark_bg_star', '#423400')
            elif b_type == 'alert':
                bg_color_hex = getattr(self, 'bookmark_bg_alert', '#420000')
            else:
                bg_color_hex = getattr(self, 'bookmark_bg_question', '#0d4200')
            
            # Получаем прозрачность из настроек (от 0.0 до 1.0)
            opacity = getattr(self, 'bookmark_bg_opacity', 0.60)
    ...
    def update_preview(self):
        ...
        # Проверяем, включен ли предпросмотр
        if not getattr(self, 'preview_enabled', True):
            return
            
        # Если предпросмотр полностью схлопнут вниз (высота 0), ставим на паузу
        sizes = self.main_vertical_splitter.sizes() if hasattr(self, 'main_vertical_splitter') else None
        if sizes and len(sizes) > 1 and sizes[1] == 0:
            self.preview_collapsed = True
            return
        ...
        try:
            for group_indices in batch:
                ...
                for idx_in_group, idx in enumerate(group_indices):
                    ...
                    # [SMART PASTE] Подключаем обработчик маркеров из превью
                    trans_edit.smart_paste_requested.connect(self.handle_smart_paste_request)
    ...
    def on_translation_changed(self):
        ...
        # [SMART MARKERS] Распознаём маркеры 🔹N🔹 для точной привязки строк
        markers_detected = False
        if '🔹' in raw_text:
            import re as _re
            # Разрезаем весь текст по маркерам (обрабатывает склейку и переносы)
            # Паттерн findall ищет все пары (номер, текст_до_следующего_маркера)
            _all_markers = _re.findall(r'🔹(\d+)🔹\s*(.*?)(?=🔹\d+🔹|$)', raw_text, _re.DOTALL)
            marker_dict = {}
            for num_str, text_block in _all_markers:
                idx = int(num_str)
                text = text_block.strip()
                # Сплющиваем физические переносы строк от ИИ в обычные пробелы,
                # чтобы не сломать 1-to-1 маппинг строк в окне перевода
                text = _re.sub(r'\s*[\r\n]+\s*', ' ', text)
                # Убираем подсказку "[добавьте \ в конце]" из переведённого текста
                text = _re.sub(r'\s*\[добавьте\s*\\+\s*в\s*конце\]', '', text)
                marker_dict[idx] = text
            
            if len(marker_dict) >= 1:  # Даже одна строка — точечная вставка
                markers_detected = True
                
                # Гарантируем что карта дедубликации существует
                self._build_dedup_map_if_needed()
                
                # Если включена оптимизация, разворачиваем дубликаты
                expanded_marker_dict = {}
                if getattr(self, 'translation_dedup_map', None):
                    for m_idx, text in marker_dict.items():
                        if m_idx in self.translation_dedup_map:
                            for real_idx in self.translation_dedup_map[m_idx]:
                                expanded_marker_dict[real_idx] = text
                        else:
                            expanded_marker_dict[m_idx] = text
                    marker_dict = expanded_marker_dict
                    
                clean_lines = []
                for i in range(len(self.original_lines)):
                    if (i + 1) in marker_dict:
                        # Маркер найден — берём новый текст
                        line_text = marker_dict[i + 1]
                    else:
                        # Маркера нет — сохраняем существующий перевод
                        line_text = self.original_lines[i].get('translated_text', '')
                    clean_lines.append(line_text)
                
                translation_lines = clean_lines
                
                # Обновляем виджет чистым текстом без маркеров
                self.prevent_text_changed = True
                ...
                self.statusBar().showMessage(
                    get_translation(self.current_language, 'status_smart_paste').format(count=len(marker_dict))
                )
        ...
        for i, line_data in enumerate(self.original_lines):
            ...
            # Обновляем флаг is_empty, чтобы фильтры корректно отображали/скрывали строку
            # (особенно если пользователь сначала вручную очистил строку, а потом вставил маркером)
            line_data['is_empty'] = not bool(new_text.strip())
    ...
    def on_preview_line_inserted(self, after_index, move_text, trans_edit_widget,
        ...
        try:
            ...
            # [SMART PASTE] Подключаем обработчик маркеров
            new_trans.smart_paste_requested.connect(self.handle_smart_paste_request)
    ...
    def _cleanup_temp_files(self):
        """Удаляет все временные файлы и папки программы из %TEMP% и сгенерированные TTS аудио."""
        try:
            temp_dir = tempfile.gettempdir()
            prefixes = ("dcs_preview_", "dcs_repl_", "dcs_drag_")
            removed_count = 0
            for entry in os.listdir(temp_dir):
                if entry.startswith(prefixes) or "_ResKey_" in entry:
            ...
            # Очистка временных файлов генерации TTS
            if getattr(sys, 'frozen', False):
                tts_dir = os.path.join(os.path.dirname(sys.executable), "tts_data")
            else:
                tts_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "tts_data")
                
            if os.path.exists(tts_dir):
                for entry in os.listdir(tts_dir):
                    if entry.startswith("preview_") and (entry.endswith(".wav") or entry.endswith(".ogg")):
                        try:
                            os.remove(os.path.join(tts_dir, entry))
                            removed_count += 1
                        except Exception:
                            pass
                            
                batch_dir = os.path.join(tts_dir, "batch")
                if os.path.exists(batch_dir):
                    for entry in os.listdir(batch_dir):
                        if entry.startswith("batch_") and (entry.endswith(".wav") or entry.endswith(".ogg")):
                            try:
                                os.remove(os.path.join(batch_dir, entry))
                                removed_count += 1
                            except Exception:
                                pass

            if removed_count > 0:
                ErrorLogger.log_info("TEMP_CLEANUP", f"Temp cleanup: removed {removed_count} temp files/dirs")
        except Exception as e:
            ErrorLogger.log_error("TEMP_CLEANUP", f"Temp cleanup error: {e}")
