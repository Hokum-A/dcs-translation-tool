...
class LuaScriptParser:
    ...
    def __init__(self):
        ...
        # trigger.action.outTextForGroup(<groupId>, <expression>, <duration>)
        self._outtext_for_re = re.compile(
            r'^(.*trigger\.action\.outTextFor(?:Group|Coalition|Country)\(\s*[^,]+,\s*)(.+?)\s*,\s*(\d+)\s*(\).*)$'
        )

        # missionCommands.addCommand("Text", ...)
        self._mission_cmd_re = re.compile(
            r'^(.*missionCommands\.(?:addCommand|addSubMenu|removeItem)\(\s*)(["\'].+?["\'])(.*)$'
        )
    ...
    def _has_translatable(self, content):
        ...
        for raw_line in content.split('\n'):
            ...
            if self._outtext_re.search(line):
                ...
                if any(s['type'] == 'text' and s['value'].strip() for s in segments):
                    return True
            if self._outtext_for_re.search(line):
                m = self._outtext_for_re.search(line)
                expr = m.group(2)
                segments = self._parse_expression(expr)
                if any(s['type'] == 'text' and s['value'].strip() for s in segments):
                    return True
            if self._mission_cmd_re.search(line):
                m = self._mission_cmd_re.search(line)
                expr = m.group(2)
                segments = self._parse_expression(expr)
                if any(s['type'] == 'text' and s['value'].strip() for s in segments):
                    return True
    ...
    def _extract_from_content(self, content):
        ...
        for i, raw_line in enumerate(lines):
            ...
            # Проверяем outTextFor*
            if not entry:
                m_outfor = self._outtext_for_re.search(line)
                if m_outfor:
                    expr = m_outfor.group(2)
                    segments = self._parse_expression(expr)
                    if any(s['type'] == 'text' and s['value'].strip() for s in segments):
                        entry = {
                            'line_num': i,
            ...
            # Проверяем missionCommands
            if not entry:
                m_cmd = self._mission_cmd_re.search(line)
                if m_cmd:
                    expr = m_cmd.group(2)
                    segments = self._parse_expression(expr)
                    if any(s['type'] == 'text' and s['value'].strip() for s in segments):
    ...
    def _parse_expression(self, expr):
        ...
        for part in parts:
            ...
            if (part.startswith('"') and part.endswith('"') and len(part) >= 2) or \
               (part.startswith("'") and part.endswith("'") and len(part) >= 2):
                # Строковый литерал — извлекаем содержимое между кавычками
                text = part[1:-1]
                segments.append({'type': 'text', 'value': text, 'quote_char': part[0]})
            else:
    ...
    def _rebuild_lua(self, content, entries, translations):
        ...
        for entry in entries:
            ...
            elif entry['pattern'] == 'outTextFor':
                new_line = self._outtext_for_re.sub(
                    lambda m: m.group(1) + new_expr + ', ' + m.group(3) + m.group(4),
                    original_line
                )
            elif entry['pattern'] == 'missionCmd':
                new_line = self._mission_cmd_re.sub(
                    lambda m: m.group(1) + new_expr + m.group(3),
                    original_line
                )
            else:
    ...
    def _build_expression(self, segments, translations):
        ...
        for seg in segments:
            if seg['type'] == 'var':
                ...
                marker = seg.get('marker_idx')
                quote = seg.get('quote_char', '"')
                if marker and marker in translations:
                    translated = translations[marker].strip()
                    # Сохраняем начальные/концевые пробелы из оригинала
                    # Они важны для конкатенации с переменными (например "Высота " .. alt)
                    original = seg['value']
                    leading = original[:len(original) - len(original.lstrip())]
                    trailing = original[len(original.rstrip()):]
                    text = self._escape_lua_string(leading + translated + trailing, quote)
                else:
                    text = seg['value']
                parts.append(f'{quote}{text}{quote}')

        return ' .. '.join(parts)

    def _escape_lua_string(self, text, quote='"'):
