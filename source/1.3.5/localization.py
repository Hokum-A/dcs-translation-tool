...
TRANSLATIONS = {
    'ru': {
        ...
        'view_log_btn': '📋 Просмотреть лог ошибок',
            ...
            'setting_preview_enabled': 'Включить предпросмотр',
            'setting_preview_tooltip': 'Если отключить, нижняя панель будет скрыта, что значительно ускорит загрузку\nфайлов с огромным числом строк (2000+). Также предпросмотр автоматически\nперестает строиться, если вы вручную перетянете разделитель в самый низ.',
        ...
        'error_optimize_requires_smart_paste': 'Оптимизация текста недоступна!\nДля ее работы необходимо включить опцию\n«Умная вставка с маркерами» в настройках.',
        ...
        'optimize_text_label': 'Оптимизация текста',
        'context_select_lang': 'Выбор шаблона:',
        'tooltip_add_context': 'Добавляет контекст в начало перевода для ИИ переводчика',
        'tooltip_optimize_text': 'Удаляет дубликаты и технический текст (цифры, координаты) перед копированием. ИИ переведет текст 1 раз, а программа применит его ко всем дубликатам!',
        'status_optimized_copy': '✅ Оптимизировано: из {total} строк скопировано {unique} уникальных фраз (сжатие {ratio}%)',
...
&nbsp;&nbsp;<code>pip install torch torchaudio --upgrade --index-url https://download.pytorch.org/whl/cu128</code><br>
• Установите движок TTS:<br>
&nbsp;&nbsp;<code>pip install coqui-tts==0.27.5 transformers==4.57.6 soundfile torchcodec</code><br>
...
• Положите их строго внутрь папки <code>tts_data/xtts/</code>.<br>
        ...
        'tts_convert_ogg': 'Сжимать в OGG',
        'tts_convert_ogg_tooltip': 'Автоматически конвертировать сгенерированный WAV файл в OGG (уменьшает размер файла миссии в 10 раз)',
        ...
        # Окно перевода Lua скриптов
        'lua_btn_text': '⚠ Перевод LUA скриптов',
        'lua_btn_tooltip': '⚠ Найдены Lua скрипты с текстом для перевода.\nСоздание новой локали не скопирует скрипты.\nРекомендуется перевод в режиме DEFAULT.',
        'lua_window_title': 'Перевод Lua скриптов',
        'lua_window_warning': '⚠ В этом окне вы можете перевести текст из Lua скриптов, найденных в миссии. Рекомендуется работать в DEFAULT с перезаписью оригинала. Lua скрипты не копируются при создании новой локали.',
        'lua_found_info': 'Найдено: {files} файлов, {unique} уникальных строк, поделено на {lines} строк для перевода',
        'lua_copy_all': '📋 Копировать весь текст',
        'lua_paste': '📥 Вставить текст из буфера',
        'lua_rewrite': '💾 Перезаписать',
        'lua_rewrite_tooltip': 'Внимание: при нажатии изменения сразу запишутся в файл миссии. Рекомендуется сделать бекап файла.',
        'lua_rewrite_confirm_title': 'Подтверждение перезаписи',
        'lua_rewrite_confirm': 'Перезаписать Lua скрипты в файле миссии?\nЭто действие изменит .miz файл.',
        'lua_save_first': 'Сначала сохраните миссию перед переводом Lua скриптов.',
        'lua_save_first_title': 'Несохранённые изменения',
        'lua_rewrite_success': '✅ Перезаписано {count} строк в {files} файлах.',
        'lua_rewrite_error': '❌ Ошибка перезаписи: {error}',
        'lua_no_translations': '⚠ Нет переведённых строк для перезаписи.\nВставьте перевод в правую панель.',
        'lua_original_label': 'Оригинал (строк: {count})',
        'lua_translation_label': 'Перевод (строк: {count})',
        'lua_status_copied': '✅ Скопировано {count} строк в буфер обмена',
        'lua_status_pasted': '✅ Текст вставлен из буфера обмена',
        'lua_status_clipboard_empty': '⚠ Буфер обмена пуст',
        'lua_scanning': 'Сканирование Lua скриптов...',
        'lua_chunk_split_label': 'Разбить на части:',
        'lua_chunk_no_data': 'Нет данных для разбиения',
        'lua_chunk_part': '{prefix}Скопировать часть {part} (строки {start} – {end})',
        'status_optimized_part_copy': '✅ Часть {part}/{parts}: из {total} строк скопировано {unique} уникальных (сжатие {ratio}%)',
        'status_part_copied': '✅ Скопирована часть {part}/{parts}: {count} строк в буфер обмена',
        'lua_chunk_disabled_smart_paste': '⚠ Включите "Умную вставку" для копирования по частям'
    },
    ...
    'en': {
        ...
        'view_log_btn': '📋 View error log',
            ...
            'setting_preview_enabled': 'Enable preview panel',
            'setting_preview_tooltip': 'If disabled, the bottom panel will be hidden, which significantly speeds up\nloading files with a huge number of lines (2000+). Also, the preview\nautomatically stops rendering if you manually drag the splitter to the very bottom.',
        ...
        'error_optimize_requires_smart_paste': 'Text optimization is unavailable!\nEnable the «Smart Paste AI» option in settings to use it.',
        ...
        'optimize_text_label': 'Optimize text',
        ...
        'tooltip_optimize_text': 'Removes duplicates and technical text (numbers, coordinates) before copying. AI translates once, and the tool applies it to all duplicates!',
        'status_optimized_copy': '✅ Optimized: from {total} lines copied {unique} unique phrases (compression {ratio}%)',
...
• Place them directly inside the <code>tts_data/xtts/</code> folder.<br>
        ...
        'tts_convert_ogg': 'Compress to OGG',
        'tts_convert_ogg_tooltip': 'Automatically convert generated WAV file to OGG (reduces mission file size by 10x)',
        ...
        # Lua script translation window
        'lua_btn_text': '⚠ LUA Scripts Translation',
        'lua_btn_tooltip': '⚠ Lua scripts with translatable text found.\nCreating a new locale will not copy scripts.\nTranslation in DEFAULT mode is recommended.',
        'lua_window_title': 'Lua Script Translation',
        'lua_window_warning': '⚠ In this window you can translate text from Lua scripts found in the mission. It is recommended to work in DEFAULT with original overwrite. Lua scripts are not copied when creating a new locale.',
        'lua_found_info': 'Found: {files} files, {unique} unique lines, split into {lines} lines for translation',
        'lua_copy_all': '📋 Copy all text',
        'lua_paste': '📥 Paste text from clipboard',
        'lua_rewrite': '💾 Rewrite',
        'lua_rewrite_tooltip': 'Warning: clicking this will immediately write changes to the mission file. It is recommended to make a backup first.',
        'lua_rewrite_confirm_title': 'Confirm rewrite',
        'lua_rewrite_confirm': 'Rewrite Lua scripts in the mission file?\nThis action will modify the .miz file.',
        'lua_save_first': 'Save the mission first before translating Lua scripts.',
        'lua_save_first_title': 'Unsaved changes',
        'lua_rewrite_success': '✅ Rewritten {count} lines in {files} files.',
        'lua_rewrite_error': '❌ Rewrite error: {error}',
        'lua_no_translations': '⚠ No translated lines to rewrite.\nPaste translation into the right panel.',
        'lua_original_label': 'Original (lines: {count})',
        'lua_translation_label': 'Translation (lines: {count})',
        'lua_status_copied': '✅ Copied {count} lines to clipboard',
        'lua_status_pasted': '✅ Text pasted from clipboard',
        'lua_status_clipboard_empty': '⚠ Clipboard is empty',
        'lua_scanning': 'Scanning Lua scripts...',
        'lua_chunk_split_label': 'Split into parts:',
        'lua_chunk_no_data': 'No data to split',
        'lua_chunk_part': '{prefix}Copy part {part} (lines {start} – {end})',
        'status_optimized_part_copy': '✅ Part {part}/{parts}: from {total} lines copied {unique} unique (compression {ratio}%)',
        'status_part_copied': '✅ Copied part {part}/{parts}: {count} lines to clipboard',
        'lua_chunk_disabled_smart_paste': '⚠ Enable "Smart Paste" to use chunked copying'
    }
}
