...
added_files = [
    ...
    ('TTScat.png', '.'),         # Used in main.py for TTS button
    ('brief.png', '.'),         # New brief icon
    ('EXIT.gif', '.'),
    ('down.gif', '.'),
    ('arrow1.gif', '.'),
    ('arrow2.gif', '.'),
    ('DSCTT.ico', '.'),
    ('xtts_server.py', '.'),      # XTTS background server script (launched by .venv Python)
    ('images/*', 'images')        # Flag icons
]
...
a = Analysis(
    ...
    hooksconfig={},
    runtime_hooks=[],
    excludes=['torch', 'TTS', 'scipy', 'numpy', 'transformers', 'torchaudio', 'librosa', 'tensorboard', 'huggingface_hub', 'tqdm', 'sklearn', 'numba', 'llvmlite', 'matplotlib'],
