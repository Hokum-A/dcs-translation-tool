...
class MizResourceManager:
    ...
    def reset(self):
        ...
        self.replaced_audio = {}
        self._file_sizes = {}  # {full_path_in_zip: size_in_bytes}
        self.pending_files = {}
    ...
    def parse_mission_audio_links(self, miz_archive=None, dictionary_keys=None, _cached_text=None):
        ...
        # Используем объединенную карту (DEFAULT + текущая локаль), чтобы не терять звуки
        ...
        if orphan_reskeys:
            ...
            # Если dictionary_keys пустой (пришел из main.py до инициализации), 
            # используем наш стандартный экстрактор.
            if not dictionary_keys:
                dictionary_keys = self._extract_dictionary_keys(miz_archive, folder=self.current_folder)
            ...
            for dk, text_val in dk_items.items():
                ...
                if m and dk not in self.subtitle_to_reskey:
                    available_dictkeys[int(m.group(2))] = (dk, bool(str(text_val).strip()))
    ...
    def _extract_dictionary_keys(self, miz_archive, folder="DEFAULT"):
        """Извлекает ключи и их текст из dictionary файла архива для конкретной локали.
        
        Returns:
            dict: {key_name: text_value} для всех DictKey_ записей
        """
        keys_dict = {}
        # Проверяем и DEFAULT (как базу) и целевую локаль
        locales_to_check = ["DEFAULT"]
        if folder != "DEFAULT":
            locales_to_check.append(folder)
            
        for loc in locales_to_check:
            dict_path = f'l10n/{loc}/dictionary'
            # Поиск файла (регистронезависимо)
            found_path = None
            for name in miz_archive.namelist():
    ...
    def load_from_miz(self, miz_archive, current_folder, dictionary_keys=None):
        ...
        if dictionary_keys is None:
            dictionary_keys = self._extract_dictionary_keys(miz_archive, folder=current_folder)
        ...
        # Кэшируем размеры всех файлов для статус-бара
        self._cache_file_sizes(miz_archive)
    
    def _cache_file_sizes(self, miz_archive):
        """Сканирует архив и кэширует размеры всех файлов."""
        self._file_sizes = {info.filename: info.file_size for info in miz_archive.infolist()}
    ...
    def update_locale(self, miz_archive, new_folder, dictionary_keys=None):
        ...
        # Автоматически извлекаем ключи из dictionary, если не переданы
        if dictionary_keys is None:
            dictionary_keys = self._extract_dictionary_keys(miz_archive, folder=new_folder)
    ...
    def get_all_resource_files(self, current_dict_keys=None):
        ...
        # Реверсивный маппинг для поиска ПЕРВОГО DictKey для каждого ResKey
        res_to_dict = {}
        for dk, rk in self.subtitle_to_reskey.items():
            if rk not in res_to_dict:
                # Фильтруем по ключам текущей локали (если переданы)
                if current_dict_keys is None or dk in current_dict_keys:
                    res_to_dict[rk] = dk
        ...
        for res_key, filename in all_map.items():
            ...
            if fname_lower.endswith(IMAGE_EXTENSIONS):
                ...
                # [FIX] Сначала проверяем физическое наличие в архиве в текущей локали, 
                # затем в DEFAULT. Это обеспечивает приоритет локали над базой.
                res_path = f"l10n/{self.current_folder}/{filename}"
                if res_path not in self._file_sizes and not in_current:
                    res_path = f"l10n/DEFAULT/{filename}"
                
                file_size = self._file_sizes.get(res_path, 0)
                ...
                result.append({
                    ...
                    'in_current_locale': in_current,
                    'size': file_size,
                    'linked_dict_key': res_to_dict.get(res_key)
                })
        ...
        for kb_img in self.kneeboard_images:
            ...
            if res_key not in seen_reskeys:
                seen_reskeys.add(res_key)
                res_path = f"KNEEBOARD/IMAGES/{kb_img}"
                file_size = self._file_sizes.get(res_path, 0)
    ...
    def resolve_to_res_key(self, key):
        """Преобразует DictKey в связанный с ним ResKey. 
        Если уже ResKey или не найден - возвращает оригинал."""
        ...
        if key.startswith("DictKey_"):
            ...
            return res_key if res_key else key
            
        return key

    def is_audio_replaced(self, key):
        """Проверяет, был ли аудиофайл заменён (ещё не сохранён)."""
        res_key = self.resolve_to_res_key(key)
        if not res_key:
            return False
            
        # Проверяем через общий метод ресурсов
    ...
    def replace_audio(self, key, source_path):
        ...
        if res_key.startswith("KneeboardKey_"):
            ...
            # 1.2 [FIX] Если этот res_key уже был заменен в этой сессии (есть в pending_files),
            # нужно удалить предыдущий временный файл и убрать его из очереди.
            if old_filename:
                old_pending_path = f"l10n/{self.current_folder}/{old_filename}"
                if old_pending_path in self.pending_files:
                    old_source = self.pending_files.pop(old_pending_path)
                    try:
                        if os.path.exists(old_source) and "dcs_repl_" in os.path.basename(old_source):
                            os.remove(old_source)
                            print(f"DEBUG: Removed intermediate pending file: {old_source}")
                    except Exception: pass

            target_path = f"l10n/{self.current_folder}/{filename}"

        # 2. Добавляем в pending_files (для всех типов ресурсов)
        if not os.path.exists(source_path):
            logger.warning(f"replace_audio: source_path does not exist: {source_path}")
            print(f"DEBUG: replace_audio failed - source does not exist: {source_path}")
            return None

        # Скопируем файл во временную директорию
        try:
            import tempfile
    ...
    def extract_resource_to_file(self, miz_path, key, output_path):
        ...
        Args:
            miz_path: путь к .miz файлу
            key: ключ ресурса (ResKey_...) или словаря (DictKey_...)
        ...
        if key.startswith("DictKey_"):
            res_info = self.get_audio_for_key(key)
        else:
            res_info = self.get_audio_for_res_key(key)
            
        if not res_info:
            logger.warning(f"extract_resource_to_file: Не найден ресурс {key}")
            return False
