...
from widgets import ToggleSwitch, ZoomablePreviewArea, JumpSlider, ClickableLabel, CustomSplitter, SearchCheckBox, NumberedTextEdit, CustomScrollBar
...
class AIContextWindow(CustomDialog):
    ...
    def setup_ui(self):
        ...
        warn_text = get_translation(self.lang, 'context_unsaved_warning')
        if hasattr(self.parent(), 'register_custom_tooltip'):
            self.parent().register_custom_tooltip(self.warn_1, warn_text)
        else:
            self.warn_1.setToolTip(warn_text)
        self.warn_1.hide()

        self.warn_2 = QLabel("⚠️", self)
        self.warn_2.setStyleSheet("font-size: 30px; color: #ffcc00;")
        if hasattr(self.parent(), 'register_custom_tooltip'):
            self.parent().register_custom_tooltip(self.warn_2, warn_text)
        else:
            self.warn_2.setToolTip(warn_text)
        self.warn_2.hide()
...
class SettingsWindow(CustomDialog):
    ...
    def __init__(self, current_language, parent=None):
        ...
        # === Ряд: Включить предпросмотр ===
        row_preview_enabled = QWidget()
        row_preview_enabled_layout = QHBoxLayout(row_preview_enabled)
        row_preview_enabled_layout.setContentsMargins(5, 0, 5, 0)
        row_preview_enabled_layout.setSpacing(10)

        self.preview_enabled_toggle = ToggleSwitch()
        row_preview_enabled_layout.addWidget(self.preview_enabled_toggle)

        self.preview_enabled_label = QLabel(get_translation(self.current_language, 'setting_preview_enabled'))
        self.preview_enabled_label.setStyleSheet('color: #ddd; background: transparent;')
        row_preview_enabled_layout.addWidget(self.preview_enabled_label)
        
        if self.parent_window and hasattr(self.parent_window, 'register_custom_tooltip'):
            self.parent_window.register_custom_tooltip(self.preview_enabled_label, get_translation(self.current_language, 'setting_preview_tooltip'))
        else:
            self.preview_enabled_label.setToolTip(get_translation(self.current_language, 'setting_preview_tooltip'))

        row_preview_enabled.setFixedHeight(28)
        row_preview_enabled_layout.addStretch()
        content_layout.addWidget(row_preview_enabled, 0, Qt.AlignTop)
    ...
    def load_values_from_parent(self):
        if self.parent_window:
            ...
            # Загружаем настройку Preview Enabled
            self.preview_enabled_toggle.setChecked(getattr(self.parent_window, 'preview_enabled', True))
            ...
            try:
                self._orig_parent_values = {
                    ...
                    'preview_enabled': bool(getattr(self.parent_window, 'preview_enabled', True))
                }
            except Exception:
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            self.preview_enabled_toggle.setChecked(True)
    ...
    def save_and_close(self):
        ...
        if self.parent_window:
            ...
            self.parent_window.preview_enabled = self.preview_enabled_toggle.isChecked()
            ...
            # Применяем настройку отображения превью
            if hasattr(self.parent_window, 'apply_preview_enabled_setting'):
                self.parent_window.apply_preview_enabled_setting()
        ...
        try:
            if self.parent_window and getattr(self, '_orig_parent_values', None):
                ...
                try:
                    ...
                    self.parent_window.preview_enabled = vals.get('preview_enabled', True)
...
class TTSPreviewDialog(CustomDialog):
    ...
    def __init__(self, text, engine, parent=None, shared_tts_duration_cache=None):
        ...
        # [NEW] Чекбокс конвертации в OGG
        self.cb_convert_ogg = QCheckBox(get_translation(self.current_language, 'tts_convert_ogg'))
        self.cb_convert_ogg.setCursor(Qt.PointingHandCursor)
        self.register_custom_tooltip(self.cb_convert_ogg, get_translation(self.current_language, 'tts_convert_ogg_tooltip'))
        ...
        # Восстанавливаем настройку OGG, по умолчанию включено
        if hasattr(self, 'settings'):
            initial_ogg = self.settings.value("tts_convert_ogg", True, type=bool)
            self.cb_convert_ogg.setChecked(initial_ogg)
        else:
            self.cb_convert_ogg.setChecked(True)
        
        # Стилизация
        checkbox_style = self.cb_filter_symbols.styleSheet()
        self.cb_auto_play.setStyleSheet(checkbox_style)
        self.cb_convert_ogg.setStyleSheet(checkbox_style)
        
        self.cb_auto_play.toggled.connect(self._on_auto_play_toggled)
        self.cb_convert_ogg.toggled.connect(self._on_convert_ogg_toggled)
        
        # Вертикальный контейнер для чекбоксов
        checkbox_v_layout = QVBoxLayout()
        checkbox_v_layout.setSpacing(2)
        checkbox_v_layout.addWidget(self.cb_auto_play)
        checkbox_v_layout.addWidget(self.cb_convert_ogg)
        
        btn_h_layout.addSpacing(15)
        btn_h_layout.addLayout(checkbox_v_layout)
        btn_h_layout.addStretch()
    ...
    def _on_convert_ogg_toggled(self, checked):
        if hasattr(self, 'settings'):
            self.settings.setValue("tts_convert_ogg", checked)
    ...
    def _get_tts_params(self):
        ...
        params = {'convert_to_ogg': self.cb_convert_ogg.isChecked()}
        if self.tabs.currentIndex() == 0:  # Piper
            engine_type = "piper"
    ...
    def _on_batch_item_finished(self, path):
        ...
        QTimer.singleShot(10, self._process_next_batch_item)
    
    def _on_batch_item_error(self, err_msg):
        """Ошибка генерации одного файла — пропускаем, продолжаем (или прерываем если критическая)."""
        res_key = self._batch_queue[self._batch_index][0]
        ...
        # Предотвращение бесконечного цикла XTTS-ошибок
        if "voice not defined" in err_msg or "server stopped responding" in err_msg.lower():
            self._batch_stop_requested = True
            print("DEBUG: Batch generation aborted due to fatal XTTS error.")
...
class LuaTranslationWindow(CustomDialog):
    """Автономное окно для перевода текста в Lua скриптах DCS миссий.
    
    Рабочий процесс:
    1. Открытие → автоматическое сканирование .miz на Lua скрипты
    2. 📋 Копировать всё → текст с маркерами в буфер обмена
    3. Перевод во внешнем инструменте (ChatGPT, DeepL и т.д.)
    4. 📥 Вставить перевод → из буфера в правую панель
    5. ⚡ Перезаписать → запись в .miz архив → окно закрывается
    """

    def __init__(self, miz_path, ai_context='', context_enabled=False, 
                 current_language='ru', parent=None):
        super().__init__(parent)
        ...
        # Данные парсера (заполняются при сканировании)
        self.lua_files = {}
        ...
        # Запускаем сканирование после показа окна
        QTimer.singleShot(100, self._scan_lua_files)

    def _setup_ui(self):
        """Создание интерфейса окна"""
        lang = self.current_language
        
        miz_name = os.path.basename(self.miz_path)
        title = f"{get_translation(lang, 'lua_window_title')} — {miz_name}"
        self.setWindowTitle(title)
        ...
        # ─── 2a. Предупреждение ───
        warning_text = get_translation(lang, 'lua_window_warning')
        warning_label = QLabel(warning_text)
        warning_label.setWordWrap(True)
        warning_label.setStyleSheet(
            "color: #ffcc00; font-size: 12px; padding: 8px 12px; "
        ...
        # ─── 2b. Статистика ───
        self.info_label = QLabel(get_translation(lang, 'lua_scanning'))
        ...
        # ─── 2c. Две панели (оригинал | перевод) ───
        panels_splitter = CustomSplitter(Qt.Horizontal)
        panels_splitter.setHandleWidth(5)
        ...
        self.left_title = QLabel(get_translation(lang, 'lua_original_label', count=0))
        ...
        self.original_text = NumberedTextEdit()
        self.original_text.setReadOnly(True)
        self.original_text.setVerticalScrollBar(CustomScrollBar())
        self.original_text.setHorizontalScrollBar(CustomScrollBar(Qt.Horizontal))
        ...
        # Нижняя панель копирования и настроек
        copy_panel_layout = QHBoxLayout()
        
        # Состояние частичного копирования
        self._chunk_count = 2       # количество частей
        self._copied_parts = set()  # уже скопированные части
        self._last_copied_part = None # последняя скопированная часть
        
        self.copy_btn = QToolButton()
        self.copy_btn.setText(get_translation(lang, 'lua_copy_all'))
        ...
        copy_panel_layout.addWidget(self.copy_btn, 0, Qt.AlignVCenter)

        toggles_container = QWidget()
        toggles_vbox = QVBoxLayout(toggles_container)
        toggles_vbox.setContentsMargins(10, 0, 0, 0)
        ...
        # 1. Добавить контекст
        row1 = QWidget()
        context_layout = QHBoxLayout(row1)
        context_layout.setContentsMargins(0, 0, 0, 0)
        context_layout.setSpacing(8)
        self._arrow_context = QLabel()
        self.add_context_toggle = ToggleSwitch()
        self.add_context_toggle.setChecked(getattr(main_app, 'lua_add_context', True) if main_app else True)
        self.add_context_toggle.toggled.connect(self._on_context_toggled)
        
        self.add_context_label = QLabel(get_translation(lang, 'add_context_label'))
        self.add_context_label.setStyleSheet("color: #ddd; background: transparent; font-size: 12px;")
        
        context_layout.addWidget(self._arrow_context)
        context_layout.addWidget(self.add_context_toggle)
        context_layout.addWidget(self.add_context_label)
        context_layout.addStretch()
        
        # 2. Оптимизировать текст
        row2 = QWidget()
        optimize_layout = QHBoxLayout(row2)
        optimize_layout.setContentsMargins(0, 0, 0, 0)
        optimize_layout.setSpacing(8)
        self._arrow_optimize = QLabel()
        self.optimize_text_toggle = ToggleSwitch()
        self.optimize_text_toggle.setChecked(getattr(main_app, 'lua_optimize_text', True) if main_app else True)
        self.optimize_text_toggle.toggled.connect(self._on_optimize_toggled)
        
        self.optimize_text_label = QLabel(get_translation(lang, 'optimize_text_label'))
        self.optimize_text_label.setStyleSheet("color: #ddd; background: transparent; font-size: 12px;")
        
        optimize_layout.addWidget(self._arrow_optimize)
        optimize_layout.addWidget(self.optimize_text_toggle)
        optimize_layout.addWidget(self.optimize_text_label)
        optimize_layout.addStretch()
        
        if main_app and hasattr(main_app, 'register_custom_tooltip'):
            main_app.register_custom_tooltip(self.add_context_label, get_translation(lang, 'tooltip_add_context'))
            main_app.register_custom_tooltip(self.optimize_text_label, get_translation(lang, 'tooltip_optimize_text'))
            
        # Обновление стрелочек
        def update_arrow_ctx(checked):
            img_name = "Left_arrow_orange.png" if checked else "Left_arrow_gray.png"
            self._arrow_context.setPixmap(QPixmap(resource_path(f"images/{img_name}")))
            
        def update_arrow_opt(checked):
            img_name = "Left_arrow_orange.png" if checked else "Left_arrow_gray.png"
            self._arrow_optimize.setPixmap(QPixmap(resource_path(f"images/{img_name}")))

        self.add_context_toggle.toggled.connect(update_arrow_ctx)
        self.optimize_text_toggle.toggled.connect(update_arrow_opt)
        
        update_arrow_ctx(self.add_context_toggle.isChecked())
        update_arrow_opt(self.optimize_text_toggle.isChecked())

        toggles_vbox.addWidget(row1)
        toggles_vbox.addWidget(row2)
        toggles_vbox.addStretch()
        
        copy_panel_layout.addWidget(toggles_container, 0, Qt.AlignVCenter)
        copy_panel_layout.addStretch()
        
        left_layout.addLayout(copy_panel_layout)
        ...
        self.right_title = QLabel(get_translation(lang, 'lua_translation_label', count=0))
        ...
        self.translation_text = NumberedTextEdit()
        self.translation_text.disable_smart_paste = True  # Отключаем перехват умной вставки для этого окна
        self.translation_text.setVerticalScrollBar(CustomScrollBar())
        self.translation_text.setHorizontalScrollBar(CustomScrollBar(Qt.Horizontal))
        ...
        """)
        self.translation_text.textChanged.connect(self._update_translation_count)
        self.translation_text.installEventFilter(self)
        right_layout.addWidget(self.translation_text)

        self.paste_btn = QPushButton(get_translation(lang, 'lua_paste'))
        self.paste_btn.setFixedSize(200, 32)
        self.paste_btn.setCursor(Qt.PointingHandCursor)
        self.paste_btn.setStyleSheet(btn_style_white)
        self.paste_btn.setEnabled(False)
        self.paste_btn.clicked.connect(self._paste_translation)
        right_layout.addWidget(self.paste_btn, 0, Qt.AlignRight)

        panels_splitter.addWidget(left_panel)
        panels_splitter.addWidget(right_panel)
        panels_splitter.setSizes([500, 500])
        content_layout.addWidget(panels_splitter, 1)
        ...
        footer.setStyleSheet(
            "QFrame { background-color: #2b2b2b; border: none; "
        ...
        # ⚡ Перезаписать
        self.rewrite_btn = QPushButton(get_translation(lang, 'lua_rewrite'))
        self.rewrite_btn.setFixedSize(160, 32)
        self.rewrite_btn.setCursor(Qt.PointingHandCursor)
        self.rewrite_btn.setStyleSheet(btn_style_orange)
        self.rewrite_btn.setEnabled(False)
        self.rewrite_btn.clicked.connect(self._rewrite)
        main_app = self.parent()
        if main_app and hasattr(main_app, 'register_custom_tooltip'):
            main_app.register_custom_tooltip(self.rewrite_btn, get_translation(lang, 'lua_rewrite_tooltip'), side='top')
        else:
            self.rewrite_btn.setToolTip(get_translation(lang, 'lua_rewrite_tooltip'))

        footer_layout.addStretch()
        footer_layout.addWidget(self.rewrite_btn)
        footer_layout.addStretch()
    ...
    def _scan_lua_files(self):
        """Сканирует .miz архив и заполняет панели"""
        lang = self.current_language
        try:
            from lua_script_parser import LuaScriptParser
            parser = LuaScriptParser()
            
            self.lua_files = parser.scan_miz(self.miz_path)
            
            if not self.lua_files:
                self.info_label.setText("⚠ Lua скрипты с переводимым текстом не найдены.")
                self.info_label.setStyleSheet(
            ...
            self.entries_by_file, self.marker_map, self.total_markers = \
                parser.extract_all(self.lua_files)
            
            # Формируем текст для левой панели
            display_text = parser.build_display_text(self.entries_by_file)
            self.original_text.setPlainText(display_text)
            
            # Обновляем статистику
            file_count = len(self.entries_by_file)
            unique_lines = sum(len(entries) for entries in self.entries_by_file.values())
            self.info_label.setText(
                get_translation(lang, 'lua_found_info',
                                files=file_count, unique=unique_lines, lines=self.total_markers)
            )
            self.left_title.setText(
                get_translation(lang, 'lua_original_label', count=self.total_markers)
            )
            
            # Активируем кнопки и сбрасываем прогресс копирования
            self._copied_parts.clear()
            self.copy_btn.setEnabled(True)
            self.paste_btn.setEnabled(True)
            self.rewrite_btn.setEnabled(True)
            
            # Сохраняем парсер для перезаписи
            self._parser = parser
            
        except Exception as e:
            self.info_label.setText(f"❌ Ошибка сканирования: {e}")
            self.info_label.setStyleSheet(
                "color: #ff6666; font-size: 12px; "
                "background: transparent; border: none;"
            )
    ...
    def _on_optimize_toggled(self, checked):
        main_app = self.parent()
        if main_app:
            main_app.lua_optimize_text = checked
            if hasattr(main_app, 'save_settings'):
                main_app.save_settings(update_preview=False, update_ui=False)

    def _on_context_toggled(self, checked):
        main_app = self.parent()
        if main_app:
            main_app.lua_add_context = checked
            if hasattr(main_app, 'save_settings'):
                main_app.save_settings(update_preview=False, update_ui=False)

    # ──────────────────────────────────────────────
    #  Меню копирования частями
    # ──────────────────────────────────────────────
    ...
    def _change_chunk_count(self, delta):
        ...
        if new_val != self._chunk_count:
            ...
            self._copied_parts.clear()  # Сброс прогресса
            self._last_copied_part = None
            # Обновляем меню на лету без закрытия
            self._chunk_count_label.setText(str(self._chunk_count))
            # Пересобираем список частей
            self.copy_menu.close()
            QTimer.singleShot(50, lambda: self.copy_btn.showMenu())
    ...
    def _copy_part(self, part_index):
        ...
        # Получаем все маркеры из оригинала
        all_markers = re.findall(r'🔹(\d+)🔹\s*(.*?)(?=🔹\d+🔹|$)', text, re.DOTALL)
        total_raw = len(all_markers)

        if total_raw == 0:
            return

        # 1. Сначала применяем глобальную оптимизацию ко всему тексту
        lines_to_chunk = []
        global_skipped = 0

        self.translation_dedup_map = {}

        if hasattr(self, 'optimize_text_toggle') and self.optimize_text_toggle.isChecked():
            has_letters_re = re.compile(r'[a-zA-Zа-яА-Я]')
            coords_re = re.compile(r'^[NSns]\s*\d+[0-9°\'\"\.,\s\-]*[EWew]\s*\d+[0-9°\'\"\.,\s\-]*$')

            text_to_id = {}
            for num_str, text_block in all_markers:
                idx = int(num_str)
                ...
                if flat_text in text_to_id:
                    primary_idx = text_to_id[flat_text]
                    self.translation_dedup_map[primary_idx].append(idx)
                    global_skipped += 1
                else:
                    text_to_id[flat_text] = idx
                    self.translation_dedup_map[idx] = [idx]
                    lines_to_chunk.append(f"🔹{idx}🔹 {clean_text}")
        else:
            for num_str, text_block in all_markers:
                lines_to_chunk.append(f"🔹{num_str}🔹 {text_block.strip()}")
        ...
        # Выводим статус
        if hasattr(self, 'optimize_text_toggle') and self.optimize_text_toggle.isChecked():
            ratio = int((global_skipped / total_raw) * 100) if total_raw > 0 else 0
            msg = get_translation(lang, 'status_optimized_part_copy',
                                  part=part_index + 1, parts=self._chunk_count,
                                  total=total_raw, unique=len(part_lines),
                                  ratio=ratio)
        ...
        main_app = self.parent()
        ai_context = getattr(main_app, 'ai_context_1', '') if main_app else getattr(self, 'ai_context', '')

        if self.add_context_toggle.isChecked() and ai_context and ai_context.strip():
            result_text = ai_context.strip() + "\n\n" + result_text
    ...
    def _build_dedup_map_if_needed(self, force=False):
        """Строит карту дедубликации по всему оригинальному тексту, если нужно"""
        if not hasattr(self, 'optimize_text_toggle') or not self.optimize_text_toggle.isChecked():
            self.translation_dedup_map = {}
            return

        if not force and getattr(self, 'translation_dedup_map', None):
            return
    ...
    def _copy_all(self):
        """Копирует весь оригинальный текст с маркерами в буфер обмена"""
        lang = self.current_language
        text = self.original_text.toPlainText()
        ...
        if hasattr(self, 'optimize_text_toggle') and self.optimize_text_toggle.isChecked():
            ...
            for num_str, text_block in all_markers:
                ...
                if flat_text in text_to_id:
                    ...
                    text_to_id[flat_text] = idx
                    self.translation_dedup_map[idx] = [idx]
                    optimized_lines.append(f"🔹{idx}🔹 {clean_text}")
                    
            text = '\n'.join(optimized_lines)
            
            ratio = int((skipped_count / self.total_markers) * 100) if self.total_markers > 0 else 0
            msg = get_translation(lang, 'status_optimized_copy', 
                                  total=self.total_markers, 
                                  unique=len(optimized_lines), 
                                  ratio=ratio)
            self.status_label.setText(msg)
        else:
            self.status_label.setText(
                get_translation(lang, 'lua_status_copied', count=self.total_markers)
            )
        
        # Добавляем AI контекст, если включён
        main_app = self.parent()
        # В LUA окне мы проверяем собственный тоггл. Текст контекста мы берём у main_app
        ai_context = getattr(main_app, 'ai_context_1', '') if main_app else getattr(self, 'ai_context', '')
        
        if self.add_context_toggle.isChecked() and ai_context and ai_context.strip():
            text = ai_context.strip() + "\n\n" + text
    ...
    def _parse_translations_from_text(self, text):
        """Извлекает переводы из переданного текста, учитывая дедубликацию"""
        import re
        translations = {}
        for match in re.finditer(r'🔹(\d+)🔹\s*(.*?)(?=🔹\d+🔹|$)', text, re.DOTALL):
            ...
            if getattr(self, 'translation_dedup_map', None) and m_idx in self.translation_dedup_map:
                for real_idx in self.translation_dedup_map[m_idx]:
                    translations[real_idx] = val
            else:
                translations[m_idx] = val
                
        return translations

    def _paste_translation(self):
        """Вставляет текст из буфера обмена в правую панель"""
        lang = self.current_language
        clipboard = QApplication.clipboard()
        text = clipboard.text()
        
        if not text.strip():
            self.status_label.setText(get_translation(lang, 'lua_status_clipboard_empty'))
            return
            
        self._build_dedup_map_if_needed()
            
        # 1. Парсим текущие переводы из правой панели
        current_translations = self._parse_translations()
        
        # 2. Парсим новые переводы из буфера обмена
        new_translations = self._parse_translations_from_text(text)
        
        # 3. Объединяем их (новые заменяют старые)
        merged_translations = current_translations.copy()
        merged_translations.update(new_translations)
        
        # 4. Перестраиваем весь текст для правой панели
        expanded_lines = []
        if getattr(self, 'translation_dedup_map', None):
            for file_path in sorted(self.entries_by_file.keys()):
                for entry in self.entries_by_file[file_path]:
                    for seg in entry['segments']:
                        marker = seg.get('marker_idx')
                        if marker is not None:
                            val = merged_translations.get(marker, seg["value"])
                            expanded_lines.append(f'🔹{marker}🔹 {val.strip()}')
        else:
            # Если оптимизация отключена, восстанавливаем по всем маркерам оригинала
            import re
            orig_text = self.original_text.toPlainText()
            for match in re.finditer(r'🔹(\d+)🔹\s*(.*?)(?=🔹\d+🔹|$)', orig_text, re.DOTALL):
                marker = int(match.group(1))
                val = merged_translations.get(marker, match.group(2))
                expanded_lines.append(f'🔹{marker}🔹 {val.strip()}')
                
        if expanded_lines:
            full_text = '\n'.join(expanded_lines)
            self.translation_text.setPlainText(full_text)
        else:
            self.translation_text.setPlainText(text)
            
        self.status_label.setText(get_translation(lang, 'lua_status_pasted'))
    ...
    def _rewrite(self):
        """Перезаписывает Lua скрипты в .miz архиве"""
        lang = self.current_language
        
        self._build_dedup_map_if_needed()
        
        # 1. Парсим переведённый текст — извлекаем маркеры
        translations = self._parse_translations()
        
        if not translations:
            self.status_label.setText(get_translation(lang, 'lua_no_translations'))
            self.status_label.setStyleSheet(
        ...
        # 2. Перезапись (без подтверждения)

        
        # 3. Выполняем перезапись
        try:
            files_modified, strings_replaced = self._parser.rewrite_miz(
                self.miz_path, translations, self.entries_by_file, self.lua_files
            )
            
            success_msg = get_translation(
                lang, 'lua_rewrite_success',
                count=strings_replaced, files=files_modified
            )
            ...
            # Обновляем оригинальное окно после перезаписи
            QTimer.singleShot(1500, self._scan_lua_files)
            
        except Exception as e:
            error_msg = get_translation(lang, 'lua_rewrite_error', error=str(e))
    ...
    def _parse_translations(self):
        """Парсит текст из правой панели, извлекая маркеры 🔹N🔹.
        
        Returns:
            dict: {marker_idx: translated_text} или пустой dict
        """
        import re as _re
        raw_text = self.translation_text.toPlainText()
        
        if not raw_text.strip():
            return {}
        
        # Ищем маркеры 🔹N🔹 — точно такая же логика как в main.py on_translation_changed
        all_markers = _re.findall(
            r'🔹(\d+)🔹\s*(.*?)(?=🔹\d+🔹|$)', raw_text, _re.DOTALL
        )
        ...
        for num_str, text_block in all_markers:
            ...
            # Сплющиваем физические переносы строк
            text = _re.sub(r'\s*[\r\n]+\s*', ' ', text)
            
            # Разворачиваем дедубликацию, если она была применена при копировании
            if getattr(self, 'translation_dedup_map', None) and m_idx in self.translation_dedup_map:
                for real_idx in self.translation_dedup_map[m_idx]:
                    if real_idx in self.marker_map:
                        translations[real_idx] = text
            else:
                if m_idx in self.marker_map:
                    translations[m_idx] = text
        
        # Если маркеры не найдены — пробуем построчный маппинг (без маркеров)
        if not translations:
            lines = [l.strip() for l in raw_text.split('\n') if l.strip()]
            # Фильтруем строки, которые похожи на заголовки файлов [filename.lua]
            lines = [l for l in lines if not (l.startswith('[') and l.endswith(']'))]
            
            if lines:
                # Маппим по порядку: первая строка → маркер 1, вторая → маркер 2, ...
                for i, line in enumerate(lines):
                    marker_idx = i + 1
                    if marker_idx in self.marker_map:
                        translations[marker_idx] = line
        
        return translations

    def eventFilter(self, obj, event):
        """Перехват событий для умной вставки (Ctrl+V)"""
        from PyQt5.QtCore import QEvent, Qt
        from PyQt5.QtGui import QKeySequence
        import re
        
        if obj == self.translation_text and event.type() == QEvent.KeyPress:
            if event.matches(QKeySequence.Paste):
                clipboard_text = QApplication.clipboard().text()
                # Если в буфере есть маркеры 🔹N🔹, мы расцениваем это как часть перевода
                if re.search(r'🔹\d+🔹', clipboard_text):
                    self._paste_translation()
                    return True # Событие перехвачено и обработано
        return super().eventFilter(obj, event)

    def _update_translation_count(self):
        """Обновляет счётчик строк в окне перевода"""
        text = self.translation_text.toPlainText()
        if not text.strip():
            count = 0
        else:
            # Считаем количество маркеров
            import re
            markers = set(re.findall(r'🔹(\d+)🔹', text))
            if markers:
                count = len(markers)
            else:
                # Фоллбэк: считаем непустые строки (без заголовков файлов)
                lines = [l.strip() for l in text.split('\n') if l.strip() and not (l.startswith('[') and l.endswith(']'))]
                count = len(lines)
        self.right_title.setText(get_translation(self.current_language, 'lua_translation_label', count=count))

    def keyPressEvent(self, event):
        """Перехватываем Enter, чтобы окно не закрывалось случайно"""
        from PyQt5.QtCore import Qt
        if event.key() in (Qt.Key_Return, Qt.Key_Enter):
            event.ignore()
            return
