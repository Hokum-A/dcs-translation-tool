...
class LinkColumnDelegate(ColorKeepingDelegate):
    """Делегат для колонки связи: рисует галочку оранжевой при наведении."""
    def paint(self, painter, option, index):
        if not index.isValid():
            return
        ...
        if text == "✔":
            ...
            color = QColor('#ff9900') if is_hover else QColor('#888888')
...
class AudioPlaylistWidget(QWidget):
    """Виджет со списком аудиофайлов в стиле DCSTT для плеера."""
    ...
    def _init_ui(self):
        ...
        if self.show_checkboxes or self.show_link_column:
            self.table.viewport().installEventFilter(self)

        # Применяем делегаты
        self.color_delegate = ColorKeepingDelegate(self.table)
        self.filename_delegate = FilenameDelegate(self.table)
        self.link_delegate = LinkColumnDelegate(self.table)
        
        for col in range(self.table.columnCount()):
            if col == self.COL_FILENAME:
                self.table.setItemDelegateForColumn(col, self.filename_delegate)
            elif self.show_link_column and col == self.COL_LINK:
                self.table.setItemDelegateForColumn(col, self.link_delegate)
            else:
    ...
    def _get_audio_duration_fast(self, filepath):
        """Быстрое получение длительности OGG/WAV без полной загрузки файла и без сторонних библиотек."""
        try:
            # 1. Пробуем быстрый встроенный парсер OGG (читает только начало и конец файла)
                ...
                with open(filepath, 'rb') as f:
                    ...
                    if idx != -1:
                        ...
                        if sample_rate > 0:
                            f.seek(0, 2)
                            size = f.tell()
                            # Ищем последний OggS заголовок с конца файла
                            f.seek(max(0, size - 8000))
                            end_data = f.read()
                            last_oggs = end_data.rfind(b'OggS')
                            if last_oggs != -1:
                                granule = int.from_bytes(end_data[last_oggs+6:last_oggs+14], byteorder='little', signed=True)
                                if granule > 0:
                                    return granule / sample_rate
            
            # 2. Пробуем быстрый встроенный парсер WAV
            elif filepath.lower().endswith('.wav'):
                import wave
                with wave.open(filepath, 'rb') as f:
                    return f.getnframes() / float(f.getframerate())
                    
            # 3. Фолбек на медленный Pygame или Mutagen
            try:
                from mutagen import File as MutagenFile
                audio_meta = MutagenFile(filepath)
                if audio_meta and audio_meta.info:
                    return audio_meta.info.length
            except ImportError:
                pass
            ...
            logger.error(f"Fast duration parse failed for {filepath}: {e}")
            return 0
    ...
    def _process_duration_queue(self):
        ...
        try:
            ...
            if row == -1:
                ...
                if self._duration_queue: self._duration_timer.start(2)
                return
            ...
            try:
                ...
                if miz_manager and miz_path:
                    ...
                    if temp_path and os.path.exists(temp_path):
                        duration = self._get_audio_duration_fast(temp_path)
                    else:
    ...
    def eventFilter(self, obj, event):
        """Перехват событий для реализации Shift+Click и смены курсора."""
                ...
                if row >= 0:
                    if col == self.COL_CHECK:
                        ...
                        if item:
                            ...
                            # Небольшой запас для удобства клика
