...
"""
    ...
    def open_files_window(self):
        ...
        if not hasattr(self, 'files_manager_window') or self.files_manager_window is None:
            ...
            self.files_manager_window = FilesWindow(self.current_language, parent=None)
    ...
    def jump_to_briefing_key(self, key_prefix):
        ...
        if hasattr(self, 'preview_key_to_group_widget'):
            ...
            if widgets:
                ...
                self.preview_scroll.ensureWidgetVisible(target_widget)
    ...
    def setup_translation_area(self, parent_container):
        ...
        self.pane_splitter.setHandleWidth(8)
    ...
    def setup_preview_panel(self, parent_container):
        ...
        # Стилизуем ручки — они будут выполнять роль разделителя
        ...
        self.bm_nav_counter.setStyleSheet("color: #cccccc; font-size: 11px; font-weight: bold; min-width: 40px;")
    ...
    # Визуальные свойства типов закладок
    BOOKMARK_VISUALS = {
        'star':     ('★', '#ff9900'),
        'question': ('?', '#4CAF50'),
        'alert':    ('!', '#FF5252'),
    }
    BOOKMARK_EMPTY = ('☆', '#666666')

    def _get_bookmark_visual(self, key):
        """Возвращает (символ, цвет) для закладки по ключу данных"""
        bm = self.bookmarks_data.get(key)
        if bm:
            return self.BOOKMARK_VISUALS.get(bm.get('type', 'star'), self.BOOKMARK_VISUALS['star'])
    ...
    def toggle_bookmark(self, key, bookmark_label, meta_widget):
        ...
        if key in self.bookmarks_data:
            ...
            self.bookmarks_data[key] = {"type": "star", "comment": ""}
    ...
    def _apply_bookmark_visual(self, key, bookmark_label, meta_widget, active):
        ...
        if active:
            symbol, color = self._get_bookmark_visual(key)
    ...
    def render_preview_batch(self):
        ...
        try:
            for group_indices in batch:
                ...
                meta_bg = '#3d4256' if is_bookmarked else bg_color
                ...
                star_text, star_color = self._get_bookmark_visual(current_key)
    ...
    def _render_virtual_preview_group(self, vgroup):
        ...
        header_line = QLabel(f"<span style='color: #888888; font-weight: bold;'>•</span> <span style='color: #8f8f8f; font-size: 11px;'>{current_key}</span>")
        header_line.setStyleSheet('border: none; background: transparent;')
        header_line.setWordWrap(True)
        meta_row_layout.addWidget(header_line, 0, Qt.AlignTop)
        
        # ДОБАВЛЕНО: Аудио-плеер для виртуальных групп
        audio_info = self.miz_resource_manager.get_audio_for_key(current_key)
        if audio_info:
            audio_filename, is_current_locale = audio_info
            audio_color = '#00cc66' if self.miz_resource_manager.is_audio_replaced(current_key) else ('#ff9900' if is_current_locale else '#888888')
            ...
            # ВНИМАНИЕ: Виртуальные группы всегда "новые", поэтому проверяем локаль
            if audio_info and not audio_info[1]:
                warning_icon = QLabel("⚠")
                warning_icon.setStyleSheet("color: #ffcc00; background-color: transparent; font-size: 16px; margin-left: 3px;")
                warning_icon.setCursor(Qt.PointingHandCursor)
                audio_controls.addWidget(warning_icon)
        ...
        try:
            ...
            if match_type == 'text_translated':
                ...
                if processed_text != old_text:
                    ...
                    self.on_search_text_changed(self.search_input.text())
                    # Если после пересчета совпадения остались, переходим к следующему (или остаемся на текущем индексе который теперь указывает на новое)
                    if self.search_matches:
                        # Пытаемся сохранить позицию или перейти к следующему
                        if self.current_match_index >= len(self.search_matches):
                            self.current_match_index = 0
                        self.search_next()
            else:
    ...
    def _save_preview_audio_to_disk(self, key):
        ...
        if file_path:
            ...
            if self.miz_resource_manager.extract_resource_to_file(miz_path, key, file_path):
                # Показываем уведомление об успехе над меткой аудио
                ...
                if label:
                    ...
                    QTimer.singleShot(3000, lambda: self.unregister_custom_tooltip(label))
            else:
