...
"""
    ...
    def open_files_window(self):
        ...
        if not hasattr(self, 'files_manager_window') or self.files_manager_window is None:
            # Создаем окно БЕЗ родителя (parent=None), чтобы оно не зависело от главного
            self.files_manager_window = FilesWindow(self.current_language, None)
            # Принудительно ставим флаг Window, чтобы окно было независимым
            self.files_manager_window.setWindowFlags(self.files_manager_window.windowFlags() | Qt.Window)
            
            # Подключаем сигнал обновления ресурсов (для замены аудио/картинок)
            if hasattr(self.files_manager_window, 'file_manager_widget'):
                self.files_manager_window.file_manager_widget.resourcesChanged.connect(self.refresh_audio_status)
        ...
        # Сохраняем позицию окна после закрытия (для последующего использования)
        self.saved_files_window_pos = (self.files_manager_window.x(), self.files_manager_window.y())
    ...
    def toggle_briefing_window(self):
        ...
        if hasattr(self, 'saved_briefing_window_pos') and self.saved_briefing_window_pos:
    ...
    def _sync_preview_widgets_for_briefing_tab(self, key):
        ...
        try:
            ...
            for i in range(min(len(edits), len(key_preview_indices))):
                ...
                new_text = self.original_lines[idx].get('translated_text', '')
            ...
            # c) Добавляем недостающие виджеты
                ...
                for i in range(len(edits), len(key_preview_indices)):
                    ...
                    color = getattr(self, 'theme_text_modified', "#ff6666") if is_mod else getattr(self, 'theme_text_saved', "#2ecc71")
    ...
    def save_settings(self, force=False, update_preview=True, update_ui=True):
        ...
        try:
            ...
            settings = {
                ...
                'briefing_window_pos': getattr(self, 'saved_briefing_window_pos', None),
    ...
    def apply_filters(self, full_rebuild=True):
        ...
        for idx, line_data in enumerate(self.all_lines_data):
            ...
            if not show_all and getattr(self, 'filter_empty_keys', True):
    ...
    def update_preview(self):
        ...
        try:
            ...
            if not self.original_lines:
                ref_data_check = getattr(self, 'reference_data', {})
                if not ref_data_check:
                    self.preview_info.setText(get_translation(self.current_language, 'preview_info', count=0))
