...
from widgets import ToggleSwitch, CustomToolTip, ClickableLabel, ZoomablePreviewArea, JumpSlider
...
class FilenameDelegate(ColorKeepingDelegate):
    ...
    def createEditor(self, parent, option, index):
        # Создаем контейнер для имитации того самого "кастомного поля"
        ...
        # Сохраняем ссылки для setEditorData/setModelData
    ...
    def setEditorData(self, editor, index):
        # Используем UserRole + 2 (оригинальное имя), если EditRole пуст или содержит отформатированный текст
    ...
    def setModelData(self, editor, model, index):
        ...
        if new_base:
            ...
            # Устанавливаем полное имя в оба роля для синхронизации
    ...
    def paint(self, painter, option, index):
        ...
        # 1. Отрисовка фона (выделение, ховер)
        # Очищаем текст в опции, чтобы стандартный метод его не рисовал
        option.text = ""
        option.widget.style().drawControl(QStyle.CE_ItemViewItem, option, painter, option.widget)
        
        # 2. Отрисовка нашего раздельного текста
        # Берем полное имя из DisplayRole (или EditRole)
        ...
        if full_name:
            ...
            # Цвет для базового имени (белый или выделенный)
            ...
            # Цвет для расширения ("маркер" - всегда приглушенный серый)
            base_w = fm.width(base)
            ext_rect = text_rect.adjusted(base_w + 4, 0, 0, 0)
            
            painter.setPen(QColor(136, 136, 136)) # Приглушенный серый
            painter.drawText(ext_rect, Qt.AlignLeft | Qt.AlignVCenter, ext)
            
        painter.restore()

    def updateEditorGeometry(self, editor, option, index):
        # Делаем поле соразмерным шрифту и центрируем по вертикали
        fm = option.fontMetrics
        h = fm.height() + 8  # Высота шрифта + отступы
        rect = option.rect
        # Центрируем rect по вертикали внутри ячейки
...
class FileManagerWidget(QWidget):
    ...
    def __init__(self, current_language, parent=None):
        ...
        self._duration_cache = {} # Кэш для длительностей аудио
