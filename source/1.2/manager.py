...
from widgets import (
    ToggleSwitch, CustomToolTip, ClickableLabel, ZoomablePreviewArea, JumpSlider,
    FileManagerStyle, CheckboxHeader, ToolTipFilter, ButtonToolTipFilter,
    SortableTableWidgetItem, NaturalSortTableWidgetItem
)
...
# TTSWorker удален отсюда, используется общая версия из tts_engine.py
# ══════════════════════════════════════════
...
class ColorKeepingDelegate(QStyledItemDelegate):
    ...
    def initStyleOption(self, option, index):
        if option.state & QStyle.State_Selected:
            ...
            # Гарантируем, что текст не станет черным при потере фокуса
...
class FilenameDelegate(ColorKeepingDelegate):
    """Делегат для колонки имен файлов с особым редактором (имя + маркер расширения)."""
    def createEditor(self, parent, option, index):
        container = QFrame(parent)
        container.setObjectName("FilenameEditorContainer")
        container.setStyleSheet("""
            QFrame#FilenameEditorContainer {
        ...
        container.extension_label = extension_label
        
        return container

    def setEditorData(self, editor, index):
        full_name = index.data(Qt.EditRole)
        if not full_name:
            full_name = index.data(Qt.UserRole + 2)
            
        base, ext = os.path.splitext(full_name or "")
        editor.line_edit.setText(base)
        editor.extension_label.setText(ext)
        editor.line_edit.setFocus()
        editor.line_edit.selectAll()

    def setModelData(self, editor, model, index):
        new_base = editor.line_edit.text().strip()
        ext = editor.extension_label.text()
        if new_base:
            new_full_name = f"{new_base}{ext}"
            model.setData(index, new_full_name, Qt.EditRole)
            model.setData(index, new_full_name, Qt.DisplayRole)

    def paint(self, painter, option, index):
        # Отрисовка с приглушенным расширением (только для этой колонки!)
        ...
        # 2. Отрисовка раздельного текста
        full_name = index.data(Qt.DisplayRole)
        if not full_name:
            full_name = index.data(Qt.EditRole) or ""
        ...
        if full_name:
            ...
            color_base = option.palette.text().color()
            if option.state & QStyle.State_Selected:
                color_base = option.palette.highlightedText().color()
            
            painter.setPen(color_base)
            painter.drawText(text_rect, Qt.AlignLeft | Qt.AlignVCenter, base)
            
            base_w = fm.width(base)
            ext_rect = text_rect.adjusted(base_w + 4, 0, 0, 0)
    ...
    def updateEditorGeometry(self, editor, option, index):
        ...
        editor.setGeometry(rect.left(), rect.top() + y_offset, rect.width(), h)
...
class BatchReplaceDialog(QDialog):
    ...
    def __init__(self, current_language, file_filter="All Files (*)", parent=None, dropped_files=None):
        super().__init__(parent)
    ...
    def _init_ui(self):
        ...
        content_layout.setContentsMargins(30, 30, 30, 0)
        ...
        toggle_layout.addStretch()
        content_layout.addWidget(toggle_row)
        ...
        content_layout.addWidget(self.suffix_container)
        ...
        content_layout.addWidget(self.single_container)
        
        content_layout.addStretch() # Прижимаем всё к верху
        ...
        bottom_bar.setStyleSheet("background-color: #2b2b2b; border-bottom-left-radius: 9px; border-bottom-right-radius: 9px;")
    ...
    def _update_visibility(self):
        # Если файлы были перетащены (Dropped), то принудительно режим Suffix и скрываем лишнее
        if self.dropped_files:
            self.mode = "suffix"
            self.suffix_container.setVisible(True)
            self.single_container.setVisible(False)
            
            # Скрываем переключатель режимов
            self.mode_toggle.setVisible(False)
            self.lbl_mode_suffix.setVisible(False)
            self.lbl_mode_single.setVisible(False)
            
            # Обновляем заголовок и описание
            count = len(self.dropped_files)
            self.setWindowTitle(get_translation(self.current_language, 'fm_br_title'))
            
            desc = get_translation(self.current_language, 'fm_br_desc_drop', count=count)
    ...
    def get_result(self):
        return {
            "mode": self.mode, # Используем self.mode вместо проверки тоггла напрямую
    ...
    def done(self, r):
        """Сохранение настроек при закрытии диалога (Apply или Cancel)."""
        settings = get_ui_settings()
        settings.setValue("fm_last_batch_prefix", self.prefix_input.text())
        settings.setValue("fm_last_batch_suffix", self.suffix_input.text())
        super().done(r)
...
class FileManagerWidget(QWidget):
    ...
    def __init__(self, current_language, parent=None, shared_duration_cache=None):
        ...
        # TTS движок
        self.tts = TTSEngine(current_language=self.current_language)
        self._tts_worker = None

        # Кэш миниатюр для режима "Содержимое"
        self._thumbnail_cache = {}
        # Используем общий кэш длительностей аудио, если передан
        self._duration_cache = self.shared_duration_cache if self.shared_duration_cache is not None else {}
        ...
        # Очередь для загрузки длительности аудио (в фоне)
        self._duration_queue = []
        self._duration_timer = QTimer(self)
        self._duration_timer.setInterval(10)
    ...
    def _init_ui(self):
        ...
        # Горячая клавиша Ctrl+A для выбора всех видимых файлов
        self.select_all_shortcut = QShortcut(QKeySequence("Ctrl+A"), self)
        self.select_all_shortcut.activated.connect(self._on_select_all_clicked)
        ...
        self.table.setColumnCount(7) # Убрали колонку с номером, добавили Инфо, теперь их 7
        ...
        self.table.setItemDelegateForColumn(self.COL_FILENAME, FilenameDelegate(self.table))
        self.table.setItemDelegateForColumn(self.COL_DESCRIPTION, ColorKeepingDelegate(self.table))
        self.table.setItemDelegateForColumn(self.COL_STATUS, ColorKeepingDelegate(self.table))
        self.table.setItemDelegateForColumn(self.COL_TYPE, ColorKeepingDelegate(self.table))
        self.table.setItemDelegateForColumn(self.COL_INFO, ColorKeepingDelegate(self.table))
        ...
        header.setSortIndicator(self.COL_TYPE, Qt.AscendingOrder) # [NEW] Начальный индикатор
        header.sectionClicked.connect(self._on_header_clicked)
        self.table.setColumnWidth(self.COL_CHECK, 50)
        self.table.setColumnWidth(self.COL_TYPE, 55)
        self.table.setColumnWidth(self.COL_INFO, 95)
        ...
        self.image_info_label.setStyleSheet("color: #aaaaaa; font-size: 12px; margin: 4px 0 2px 0; background: transparent; border: none;")
        ...
        # Слайдер позиции
        self._ap_position_slider = JumpSlider(Qt.Horizontal)
        self._ap_position_slider.setFocusPolicy(Qt.NoFocus)
        ...
        self._ap_volume_slider = JumpSlider(Qt.Horizontal)
        self._ap_volume_slider.setFocusPolicy(Qt.NoFocus)
    ...
    def _init_status_bar(self, parent_layout):
        """Создает статусную строку внизу менеджера."""
        status_widget = QWidget()
        status_widget.setFixedHeight(24)
        status_widget.setStyleSheet("background-color: #2b2d30; border-top: 1px solid #444;")
        status_layout = QHBoxLayout(status_widget)
        status_layout.setContentsMargins(10, 0, 10, 0)
        status_layout.setSpacing(20)

        lbl_style = "color: #aaaaaa; font-size: 12px; font-weight: bold; border: none; background: transparent;"
        val_style = "color: #ff9900; font-size: 12px; font-weight: bold; border: none; background: transparent;"
        ...
        self.lbl_status_total = QLabel("-")
        self.lbl_status_total.setStyleSheet(lbl_style)
        total_layout.addWidget(self.lbl_status_total)
        status_layout.addWidget(total_container)
        ...
        self.lbl_status_selection.setStyleSheet(val_style)
        sel_layout.addWidget(self.lbl_status_selection)
        status_layout.addWidget(sel_container)

        # Инструменты выделения (кнопки в виде текста)
        ...
        self.btn_status_select_all = QPushButton(get_translation(self.current_language, 'fm_btn_select_all'))
        self.btn_status_select_all.setStyleSheet(link_style)
        self.btn_status_select_all.setCursor(Qt.PointingHandCursor)
        self.btn_status_select_all.clicked.connect(self._on_select_all_clicked)
        status_layout.addWidget(self.btn_status_select_all)
        
        self.btn_status_select_none = QPushButton(get_translation(self.current_language, 'fm_btn_select_none'))
        self.btn_status_select_none.setStyleSheet(link_style)
        self.btn_status_select_none.setCursor(Qt.PointingHandCursor)
        self.btn_status_select_none.clicked.connect(self._on_deselect_all_clicked)
        status_layout.addWidget(self.btn_status_select_none)
        
        self.btn_status_select_invert = QPushButton(get_translation(self.current_language, 'fm_btn_select_invert'))
        self.btn_status_select_invert.setStyleSheet(link_style)
        self.btn_status_select_invert.setCursor(Qt.PointingHandCursor)
        self.btn_status_select_invert.clicked.connect(self._on_invert_selection_clicked)
        status_layout.addWidget(self.btn_status_select_invert)
    ...
    def _on_select_all_clicked(self):
        """Выбирает все отображаемые файлы."""
        self._on_header_checkbox_toggled(Qt.Checked)

    def _on_deselect_all_clicked(self):
        """Снимает выделение со всех отображаемых файлов."""
        self._on_header_checkbox_toggled(Qt.Unchecked)

    def _on_invert_selection_clicked(self):
        """Инвертирует выделение для отображаемых файлов."""
        self.table.blockSignals(True)
        for r in range(self.table.rowCount()):
            it = self.table.item(r, self.COL_CHECK)
            if it:
    ...
    def eventFilter(self, obj, event):
        ...
        if obj == self.table.viewport():
            ...
            if event.type() == QEvent.MouseButtonPress:
                ...
                if row >= 0:
                    # 1. Чекбокс (указующий перст с точным хитбоксом)
                    if col == self.COL_CHECK:
                        item = self.table.item(row, col)
                        if item:
                            opt = QStyleOptionViewItem()
                            opt.initFrom(self.table.viewport())
                            opt.rect = self.table.visualRect(self.table.model().index(row, col))
                            opt.features = QStyleOptionViewItem.HasCheckIndicator
                            
                            check_rect = self.table.style().subElementRect(
                                QStyle.SE_ItemViewItemCheckIndicator, opt, self.table.viewport()
                            )
                            hitbox = check_rect.adjusted(-2, -2, 2, 2)
                            
                            if hitbox.contains(pos):
                                self.table.viewport().setCursor(Qt.PointingHandCursor)
                                cursor_changed = True
                    
                    # 2. Иконка типа (открытая ладонь для Drag-Out)
            ...
            # 2. Обработка Drag & Drop
                ...
                if event.mimeData().hasUrls():
                    ...
                    file_paths = [u.toLocalFile() for u in urls if u.toLocalFile() and os.path.isfile(u.toLocalFile())]
                    if not file_paths:
                        return True

                    # --- СЛУЧАЙ 1: ПЕРЕТАЩЕНО НЕСКОЛЬКО ФАЙЛОВ ---
                    if len(file_paths) > 1:
                        # Открываем диалог пакетной замены в режиме Dropped
                        dialog = BatchReplaceDialog(self.current_language, parent=self, dropped_files=file_paths)
                        if dialog.exec_() != QDialog.Accepted:
                            return True
                            
                        result = dialog.get_result()
                        prefix = result.get("prefix", "").lower()
                        suffix = result.get("suffix", "").lower()
                        
                        # Собираем пул файлов (имя -> полный путь) для быстрого поиска
                        file_pool = {}
                        for p in file_paths:
                            file_pool[os.path.basename(p).lower()] = p
                            
                        success_count = 0
                        # Определяем список ресурсов для обработки: 
                        # Если есть отмеченные - только они. Если нет - все видимые файлы.
                        target_keys = list(self.checked_keys) if self.checked_keys else [f['res_key'] for f in self._get_filtered_files()]
                        
                        replaced_keys = []
                        for res_key in target_keys:
                            res_info = self.miz_resource_manager.get_audio_for_res_key(res_key)
                            if not res_info: continue
                            
                            miz_filename, _ = res_info
                            pure_name = os.path.basename(miz_filename)
                            base, ext = os.path.splitext(pure_name)
                            
                            # Генерируем искомое имя (новое имя файла в Windows)
                            # Правило: basename(dropped) == prefix + base(mission) + suffix + ext
                            target_name = f"{prefix}{base}{suffix}{ext}".lower()
                            
                            if target_name in file_pool:
                                new_path = file_pool[target_name]
                                if self.miz_resource_manager.replace_audio(res_key, new_path):
                                    success_count += 1
                                    replaced_keys.append(res_key)
                                    
                        if success_count > 0:
                            # Очищаем кэши и обновляем UI
                            for rk in replaced_keys:
                                if rk in self._thumbnail_cache: del self._thumbnail_cache[rk]
                                if rk in self._duration_cache: del self._duration_cache[rk]
                            
                            self._all_files = self.miz_resource_manager.get_all_resource_files()
                            self.resourcesChanged.emit()
                            self._apply_filters() # Полная перерисовка для пакета надежнее
                            
                            # Уведомление
                            msg = get_translation(self.current_language, 'fm_batch_replace_success').format(
                                count=success_count, total=len(target_keys), path=""
                            )
                            # StandardInfoDialog - вспомогательный метод или импортирован?
                            # Он импортирован из dialogs в начале файла.
                            StandardInfoDialog(get_translation(self.current_language, 'fm_btn_batch_replace'), msg, self.current_language, parent=self).exec_()
                        else:
                            msg = get_translation(self.current_language, 'fm_batch_replace_zero').format(
                                total=len(target_keys), path=""
                            )
                            StandardInfoDialog(get_translation(self.current_language, 'fm_btn_batch_replace'), msg, self.current_language, parent=self).exec_()
                    ...
                    # --- СЛУЧАЙ 2: ПЕРЕТАЩЕН ОДИН ФАЙЛ (старая логика) ---
                    ...
                    if row >= 0:
                        ...
                        if item:
                            ...
                            if new_filename:
                                ...
                                # Очищаем кэш миниатюр и длительностей
                                for rk in [res_key, selected_res_key]:
                                    if rk in self._thumbnail_cache:
                                        del self._thumbnail_cache[rk]
                                    if rk in self._duration_cache:
                                        del self._duration_cache[rk]
                                    
                                self._all_files = self.miz_resource_manager.get_all_resource_files()
                                self.resourcesChanged.emit()
                                
                                # Плавное обновление строки вместо _apply_filters()
                                file_info = next((f for f in self._all_files if f['res_key'] == selected_res_key), None)
                                if file_info:
                                    self._update_single_row(row, file_info)
                                else:
                                    self._apply_filters() # Fallback если что-то пошло не так
    ...
    def _on_item_changed(self, item):
        ...
        if item.column() == self.COL_CHECK:
            ...
            # Используем EditRole для получения чистого полного имени после сохранения из делегата
            new_name = item.data(Qt.EditRole)
            if not new_name:
                new_name = item.text()
            ...
            if res_key and new_name and new_name != old_name:
                ...
                if new_res_key:
                    ...
                    # Плавное обновление строки вместо _apply_filters()
                    file_info = next((f for f in self._all_files if f['res_key'] == new_res_key), None)
                    if file_info:
                        self._update_single_row(item.row(), file_info)
        ...
        if index.column() == self.COL_FILENAME:
            ...
            if action == rename_action:
                ...
                if main_window:
                    ...
                    # Закрываем менеджер
                    self.window().close()
                    # Вызываем переход к ключу в главном окне
                    QTimer.singleShot(200, lambda k=linked_dict_key: main_window.jump_to_dict_key(k))
    ...
    def _start_tts_download(self):
        ...
        progress = MizProgressDialog(
            ...
            get_translation(self.current_language, 'tts_downloading_msg')
        )
        progress.show()
        ...
        if success:
            ...
            msg = StandardInfoDialog(
                get_translation(self.current_language, 'tts_install_success_title'),
                get_translation(self.current_language, 'tts_install_success_msg'),
            ...
            from localization import get_translation
            msg = StandardInfoDialog(
                get_translation(self.current_language, 'tts_install_error_title'),
                get_translation(self.current_language, 'tts_install_error_msg'),
    ...
    def _update_header_checkbox(self):
        ...
        # [FIX] Также обновляем статусну строку здесь, так как это общий метод
        self._update_status_info()
    ...
    def _update_status_info(self):
        """Обновляет информацию в статусной строке."""
        if not hasattr(self, 'lbl_status_total'): return
        ...
        for r in range(self.table.rowCount()):
            ...
            if it and it.checkState() == Qt.Checked:
                selected_count += 1
                # Размер хранится в UserRole + 1 айтема чекбокса
                total_size += it.data(Qt.UserRole + 1) or 0
                
        # Форматирование размера
        size_str = self._format_size(total_size)
        
        shown_text = get_translation(self.current_language, 'fm_status_shown', 
                                    current=visible_count, total=total_count)
        selected_text = get_translation(self.current_language, 'fm_status_selected', 
                                       count=selected_count, size=size_str)
        
        self.lbl_status_total.setText(shown_text)
        self.lbl_status_selection.setText(selected_text)
    ...
    def _process_duration_queue(self):
        """Обрабатывает по одному аудиофайлу из очереди для получения длительности."""
        try:
            if not self or not hasattr(self, '_duration_queue') or not self._duration_queue:
                return
            ...
            row = -1
            # Сначала проверяем ту же строку (оптимизация)
            test_item = self.table.item(orig_row, self.COL_FILENAME)
            if test_item and test_item.data(Qt.UserRole) == res_key:
                row = orig_row
            else:
                # Если не совпало — ищем по всей таблице
                for r in range(self.table.rowCount()):
                    it = self.table.item(r, self.COL_FILENAME)
                    if it and it.data(Qt.UserRole) == res_key:
                        row = r
            ...
            if row == -1:
                # Файл больше не отображается (отфильтрован или удален)
                if self._duration_queue:
                    self._duration_timer.start(2)
                return

            # Извлекаем и получаем длительность
            try:
                temp_path = self._extract_resource_to_temp(res_key, filename)
                duration = 0
                if temp_path and os.path.exists(temp_path):
                    try:
                        # Используем pygame.mixer.Sound только если это аудио
                        ...
                        logger.debug(f"Pygame duration fail for {filename}: {e}")
                
                # Обновляем ячейку Инфо
                ...
                if info_item:
                    ...
                    info_item.sort_value = duration # Для правильной сортировки по секундам
            ...
            # Запускаем таймер для следующей итерации (2мс для скорости)
            if self._duration_queue:
                self._duration_timer.start(2)
        except RuntimeError:
            return
    ...
    def _get_thumbnail(self, res_key, filename):
        ...
        # Добавляем серую рамку (border) вокруг эскиза
    ...
    def set_data(self, miz_resource_manager, miz_path):
        ...
        # [FIX] Очищаем кэш миниатюр и длительностей, если сменилась миссия ИЛИ локаль.
        # Это необходимо, так как файлы с одинаковыми именами (res_key) могут отличаться в разных локалях.
        current_folder = miz_resource_manager.current_folder if miz_resource_manager else None
        if getattr(self, 'miz_path', None) != miz_path or self._last_folder != current_folder:
    ...
    def _populate_table(self):
        ...
        for row, item in enumerate(filtered):
            ...
            check_item.setData(Qt.UserRole + 1, item.get('size', 0)) # Сохраняем размер файла
            if is_checked:
            ...
            # Инфо (размер для картинок, длительность для аудио)
            info_text = ""
            sort_val = 0
            if item['type'] == 'image':
                ...
                if size_bytes > 1024 * 1024:
                    info_text = f"{size_bytes / (1024*1024):.1f} MB"
                elif size_bytes > 0:
                    info_text = f"{size_bytes / 1024:.1f} KB"
                else:
                    info_text = "0 KB"
            elif item['type'] == 'audio':
                if item['res_key'] in self._duration_cache:
                    sort_val, info_text = self._duration_cache[item['res_key']]
                else:
                    info_text = "⏳" # Песочные часы как плейсхолдер
                    sort_val = 0
                    self._duration_queue.append((row, item['res_key'], item['filename']))
            ...
            info_item.setData(Qt.UserRole + 3, '#888888')
        ...
        # Статус (цветная точка)
            ...
            font.setPointSize(18) # Увеличено для лучшей видимости
            font.setBold(True)     # Сделано жирным для акцента
        ...
        # [FIX] Возвращаем отрисовку ПЕРЕД сортировкой, чтобы sortByColumn сработал корректно
        self.table.viewport().setUpdatesEnabled(True)
        self.table.setUpdatesEnabled(True)
        self.table.blockSignals(False)

        # Восстанавливаем сортировку (разово через sortByColumn для обновления стрелок)
        if current_sort_col >= 0:
            self.table.setSortingEnabled(True)
            self.table.sortByColumn(current_sort_col, current_sort_order)
        elif self._first_load and self.table.rowCount() > 0:
            # Если это первая загрузка, сортируем по типу (чтобы картинки были сверху)
            self.table.setSortingEnabled(True)
            self.table.sortByColumn(self.COL_TYPE, Qt.AscendingOrder)
            self._first_load = False
            
        # Гарантируем, что динамическая сортировка выключена, но стрелки видны
        self.table.setSortingEnabled(False)
        self.table.horizontalHeader().setSortIndicatorShown(True)
        self.table.viewport().update()
        
        # [FIX] Обновляем состояние чекбокса в заголовке и статусную строку
        self._update_header_checkbox()
        ...
        # Запускаем фоновую загрузку длительностей аудио
        if self._duration_queue:
    ...
    def retranslate_ui(self, current_language):
        """Обновляет переводы интерфейса виджета."""
        self.current_language = current_language
        self.setWindowTitle(get_translation(current_language, 'fm_title'))
        
        # Обновляем тексты в UI (кнопки, метки и т.д.)
        ...
        if hasattr(self, 'btn_export_all'):
            self.btn_export_all.setText(get_translation(current_language, 'btn_export_all'))
        if hasattr(self, 'search_input'):
            self.search_input.setPlaceholderText(get_translation(current_language, 'search_placeholder'))
            
        # Обновляем заголовки таблицы
        headers = [
            "", # Check
            get_translation(current_language, 'col_type'),
            get_translation(current_language, 'col_filename'),
            get_translation(current_language, 'col_info'),
            get_translation(current_language, 'col_description'),
            get_translation(current_language, 'col_status'),
            get_translation(current_language, 'col_actions')
        ]
        ...
        if hasattr(self, 'preview_area'):
            # Метод _update_preview_ui обычно использует переводы
    ...
    def _update_single_row(self, row, item):
        """Обновляет ячейки строки (имя, размер/инфо, статус, res_key) без перестроения всей таблицы."""
        self.table.blockSignals(True)
        try:
            res_key = item['res_key']
            
            # 0. Обновляем res_key во всех ячейках, где он хранится
            for col in [self.COL_CHECK, self.COL_FILENAME]:
                it = self.table.item(row, col)
                if it:
                    it.setData(Qt.UserRole, res_key)

            # 1. Обновляем имя файла
            fname_item = self.table.item(row, self.COL_FILENAME)
            if fname_item:
                fname_item.setText(item['filename'])
                fname_item.setData(Qt.UserRole + 2, item['filename'])

            # 2. Обновляем инфо (размер/длительность)
            info_item = self.table.item(row, self.COL_INFO)
            if info_item:
                info_text = ""
                sort_val = 0
                if item['type'] == 'image':
                    ...
                    if item['res_key'] in self._duration_cache:
                        ...
                        info_text = "⏳"
                        sort_val = 0
                        if not any(q[1] == item['res_key'] for q in getattr(self, '_duration_queue', [])):
                            self._duration_queue.append((row, item['res_key'], item['filename']))
                            if not self._duration_timer.isActive():
                                self._duration_timer.start(5)
                
                info_item.setText(info_text)
                if hasattr(info_item, 'sort_value'):
            ...
            if status_item:
                ...
                if hasattr(status_item, 'sort_value'):
                    status_item.sort_value = sort_weight
                status_item.setData(Qt.UserRole + 3, color)
                status_item.setForeground(QColor(color))
                status_item.setData(Qt.ToolTipRole, get_translation(self.current_language, tooltip_key))
                
            # 4. Обработка миниатюр для изображений
            if item['type'] == 'image' and item['res_key'] not in self._thumbnail_cache:
                type_item = self.table.item(row, self.COL_TYPE)
                if type_item and not any(q[1] == item['res_key'] for q in getattr(self, '_thumbnail_queue', [])):
                    type_item.setIcon(QIcon())
                    self._thumbnail_queue.append((type_item, item['res_key'], item['filename']))
                    self._process_thumbnail_queue()
        finally:
    ...
    def _on_header_clicked(self, logical_index):
        ...
        # Можно игнорировать клик по чекбоксу
        ...
        if header.sortIndicatorSection() == logical_index:
            new_order = Qt.DescendingOrder if header.sortIndicatorOrder() == Qt.AscendingOrder else Qt.AscendingOrder
        else:
    ...
    def _on_table_selection_changed(self):
        ...
        for row in range(self.table.rowCount()):
            ...
            # Переопределяем цвета для текста (исключаем чекбокс и статус)
            cols_to_update = [self.COL_FILENAME, self.COL_DESCRIPTION, self.COL_TYPE, self.COL_INFO]
            for col in cols_to_update:
                ...
                if item:
                    ...
                    if is_selected and col in [self.COL_FILENAME, self.COL_DESCRIPTION]:
        ...
        if self._preview_playing:
            ...
            if self._preview_paused:
                ...
                try:
                    ...
                    # Проверяем не совпадает ли текущий offset с тем, что на ползунке
                    if abs(self._preview_play_offset - self._ap_position_slider.value()) > 500:
                         self._preview_play_offset = self._ap_position_slider.value()
                    ...
                    try:
                        pygame.mixer.music.load(self._preview_audio_path)
                        self._preview_audio_loaded = True
                        pygame.mixer.music.play(start=self._ap_position_slider.value() / 1000.0)
                    except Exception:
                        pass
            else:
    ...
    def _on_preview_slider_released(self):
        ...
        try:
            ...
            self._update_preview_time_labels(val_ms)
        ...
        try:
            ...
            if self._preview_playing:
                if not self._preview_slider_dragged:
                    try:
                        # Получаем позицию. get_pos() возвращает время с начала play() в мс.
                        # Если play() только вызван, get_busy() может быть False долю секунды.
                        is_busy = pygame.mixer.music.get_busy()
                        pos = pygame.mixer.music.get_pos()
                        
                        if is_busy and pos >= 0:
                            current_ms = pos + self._preview_play_offset
    ...
    def _on_replace_clicked(self, res_key):
        ...
        if path:
            ...
            if new_filename: # Метод универсален для ресурсов
                ...
                # Обновляем in-place без перестроения таблицы
                file_info = next((f for f in self._all_files if f['res_key'] == selected_res_key), None)
                if file_info:
                    for r in range(self.table.rowCount()):
                        it = self.table.item(r, self.COL_FILENAME)
                        if it and it.data(Qt.UserRole) == selected_res_key:
                            self._update_single_row(r, file_info)
                            self.table.selectRow(r)
                            self.table.setFocus()
                            break
                    self.table.viewport().update()
        ...
        if success_count > 0:
            ...
            for res_key in list(self.checked_keys):
                ...
                if res_key in self._duration_cache:
                    del self._duration_cache[res_key]
            self._all_files = self.miz_resource_manager.get_all_resource_files()
            self.resourcesChanged.emit()
            
            # Обновляем in-place
            for res_key in list(self.checked_keys):
                file_info = next((f for f in self._all_files if f['res_key'] == res_key), None)
                if file_info:
                    for r in range(self.table.rowCount()):
                        it = self.table.item(r, self.COL_FILENAME)
                        if it and it.data(Qt.UserRole) == res_key:
    ...
    def _extract_resource_to_temp(self, res_key, filename, target_dir=None):
        ...
        # [FIX] Всегда сначала пробуем путь в текущей локали, даже если res_key 
        # не прописан в mapResource для этой локали (DCS позволяет перекрывать файлы физически).
        if res_key.startswith("KneeboardKey_"):
            target_path = f"KNEEBOARD/IMAGES/{filename}"
        else:
            # Сначала пробуем текущую локаль
            target_path = f"l10n/{mgr.current_folder}/{filename}"
            
            # Проверяем pending_files (они всегда сохраняются в текущую локаль)
            ...
            # Если файла нет в текущей локали (в архиве), и он не прописан в map_resource_current, 
            # будем проверять DEFAULT в блоке извлечения из ZIP ниже.
            # Если res_key прописан только в DEFAULT, то target_path здесь будет l10n/LOCALE/..., 
            # но z.namelist() его не найдет, и сработает fallback к l10n/DEFAULT/ в строке 3659.
    ...
    def retranslate_ui(self, current_language):
        ...
        # Обновляем инструменты выделения в статус-баре
        if hasattr(self, 'btn_status_select_all'):
            self.btn_status_select_all.setText(get_translation(current_language, 'fm_btn_select_all'))
        if hasattr(self, 'btn_status_select_none'):
            self.btn_status_select_none.setText(get_translation(current_language, 'fm_btn_select_none'))
        if hasattr(self, 'btn_status_select_invert'):
            self.btn_status_select_invert.setText(get_translation(current_language, 'fm_btn_select_invert'))

        # Обновляем числовую информацию в статус-баре (с учетом нового языка)
        self._update_status_info()
