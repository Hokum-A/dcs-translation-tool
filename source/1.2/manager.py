...
from PyQt5.QtWidgets import (
    ...
    QProxyStyle, QStyleOptionHeader, QInputDialog, QMessageBox,
    QRadioButton, QDialog, QFrame, QStackedWidget, QApplication, QMenu
)
from PyQt5.QtCore import Qt, pyqtSignal, QTimer, QObject, QEvent, QPoint, QSize, QRect, QSettings, pyqtProperty, QEasingCurve, QPropertyAnimation, QMimeData, QUrl
from PyQt5.QtGui import QPixmap, QColor, QIcon, QFont, QCursor, QPainter, QPen, QBrush, QPainterPath, QPolygon, QFontMetrics, QDrag
...
class BatchReplaceDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        self.suffix_input.setPlaceholderText("_new")
...
class StandardInfoDialog(QDialog):
    """Кастомный диалог уведомления в стиле приложения."""
    def __init__(self, message, current_language, parent=None):
        super().__init__(parent)
        self.current_language = current_language
        self.setWindowFlags(Qt.FramelessWindowHint | Qt.WindowStaysOnTopHint | Qt.Window)
        self.setAttribute(Qt.WA_TranslucentBackground)
        
        # Динамический расчет ширины, чтобы текст умещался в одну строку
        font = QFont()
        font.setBold(True)
        font.setPixelSize(14)
        fm = QFontMetrics(font)
        
        # Очищаем HTML для корректного замера самой длинной строки
        clean_msg = message.replace("<br/>", "\n").replace("<span style='color: #ff9900;'>", "").replace("</span>", "")
        max_line_width = max(fm.horizontalAdvance(line) for line in clean_msg.split('\n'))
        
        # +60 отступы (padding+margin), +20 запас, +10 по просьбе пользователя = +90
        target_width = max(440, max_line_width + 90)
        
        self.setFixedSize(target_width, 200)
        ...
        container = QFrame()
        container.setObjectName("InfoContent")
        container.setStyleSheet("""
            QFrame#InfoContent {
                background-color: #333333;
                border: 1px solid #ff9900;
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            # Разрешаем перетаскивание только если нажали на пустую область, а не на кнопки
            child = self.childAt(event.pos())
            is_interactive = False
            temp = child
            while temp:
                if temp.__class__.__name__ in ["QPushButton"]:
                    is_interactive = True
                    break
                temp = temp.parentWidget()
...
class FileManagerWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        self.preview_container.setMinimumWidth(512 + 20)
        self.preview_container.setMaximumWidth(512 + 20)
        self.preview_container.setStyleSheet(
            "background-color: #1e1e1e; border: 1px solid #3d4256; border-radius: 4px;")
        preview_layout = QVBoxLayout(self.preview_container)
        preview_layout.setContentsMargins(10, 10, 10, 10)

        # ─── Область превью (используем стопку для стабильности лейаута) ───
        self.preview_stack = QStackedWidget(self.preview_container)
        self.preview_stack.setFixedSize(512, 512)
        ...
        self._ap_position_slider.setStyleSheet("""
            QSlider::groove:horizontal { border: 1px solid #777; height: 4px; background: #b0b0b0; margin: 2px 0; border-radius: 2px; }
            ...
            QSlider::sub-page:horizontal { background: #ff9900; border-radius: 2px; }
        """)
        ...
        self._ap_stop_btn.clicked.connect(self._stop_preview_audio)
        ...
        self._ap_volume_slider.setValue(50)
        ...
        table_icon = QLabel(ICON_VIEW_TABLE)
        table_icon.setStyleSheet(f"color: #aaaaaa; font-size: {ICON_VIEW_TABLE_SIZE}px; background: transparent; border: none;")
        table_icon.setFixedWidth(24)
        table_icon.setAlignment(Qt.AlignCenter)
        slider_row.addWidget(table_icon)

        # Слайдер
        self.view_slider = QSlider(Qt.Horizontal)
        self.view_slider.setRange(0, 9)  # 0 = таблица, 1..9 = 20..100 px
        ...
        content_icon = QLabel(ICON_VIEW_CONTENT)
        # Увеличиваем значок Содержимое согласно настройке
        content_icon.setStyleSheet(f"color: #aaaaaa; font-size: {ICON_VIEW_CONTENT_SIZE}px; background: transparent; border: none;")
        content_icon.setFixedWidth(32)
        content_icon.setAlignment(Qt.AlignCenter)
        slider_row.addWidget(content_icon)
    ...
    def _on_item_changed(self, item):
        ...
        if item.column() == self.COL_CHECK:
            ...
            if res_key and new_name and new_name != old_name:
                ...
                if self.miz_resource_manager.rename_resource(res_key, new_name, self.miz_path):
                    ...
                    self._apply_filters()
                    # Восстанавливаем фокус на эту строку
                    self._restore_table_focus_by_key(res_key)
                    self.table.blockSignals(False)
                else:
    ...
    def _on_view_slider_changed(self, value):
        ...
        if value == 0:
            ...
            thumb_size = 10 + value * 10  # 20, 30, 40 … 100
    ...
    def _get_thumbnail(self, res_key, filename):
        ...
        # Генерируем миниатюру максимального размера (100x100), Qt будет уменьшать через setIconSize
    ...
    def set_data(self, miz_resource_manager, miz_path):
        ...
        # Очищаем кэш миниатюр ТОЛЬКО при смене миссии
        if not hasattr(self, 'miz_path') or self.miz_path != miz_path:
        ...
        # При первом открытии окна в текущей сессии сортируем "картинки выше"
        if self._first_load:
    ...
    def _populate_table(self):
        ...
        thumb_size = 0 if slider_val == 0 else (10 + slider_val * 10)
        ...
        # Статус (цветная точка)
            ...
            if self.miz_path and self.miz_resource_manager:
                cur_folder = self.miz_resource_manager.current_folder
                mods = self.miz_resource_manager.modified_map_resources.get(cur_folder, {})
                is_replaced = item['res_key'] in mods
    ...
    def _get_filtered_files(self):
        ...
        for item in self._all_files:
            ...
            if item['type'] == 'audio' and not show_audio:
                continue
            if item['type'] == 'image' and not show_images:
                continue
            # Фильтр по поиску
    ...
    def _set_preview_volume(self, value):
        ...
        try:
            ...
            pass
        # Синхронизируем с основным окном
        main_win = self._get_main_window()
        if main_win and hasattr(main_win, 'audio_volume'):
            main_win.audio_volume = value
            if hasattr(main_win, 'save_settings'):
                main_win.save_settings(update_ui=False)
    ...
    def _on_replace_clicked(self, res_key):
        ...
        if path:
            ...
            # Запоминаем текущий res_key для восстановления выделения
            selected_res_key = res_key
            
            if self.miz_resource_manager.replace_audio(res_key, path): # Метод универсален для ресурсов
    ...
    def _on_batch_export_clicked(self):
        ...
        if dir_path:
            ...
            dialog = StandardInfoDialog(msg, self.current_language, self)
            dialog.exec_()
    ...
    def _extract_resource_to_temp(self, res_key, filename, target_dir=None):
        ...
        try:
            if target_dir:
                ...
                # Для предпросмотра используем суффикс для избежания конфликтов
