...
class CustomDialog(QDialog):
    ...
    def paintEvent(self, event):
        ...
        # Отключаем сглаживание для самой рамки для максимальной четкости в 1 пиксель
        ...
        # Используем QRectF и смещение 0.5 для четкой отрисовки без размытия
    ...
    def set_volume(self, value):
        ...
        if self.parent() and hasattr(self.parent(), 'audio_volume'):
            ...
            if hasattr(self.parent(), 'save_settings'):
                self.parent().save_settings(update_ui=False)
...
class FilesWindow(CustomDialog):
    ...
    def keyPressEvent(self, event):
        """Обработка нажатия Enter для открытия предпросмотра вместо закрытия окна"""
        # Если нажат Enter/Return и фокус на таблице или поиске - имитируем двойной клик
            ...
            if hasattr(self, 'file_manager_widget'):
                ...
                if table.hasFocus() or search.hasFocus():
                    ...
                    if current_idx.isValid():
                        # Вызываем метод FileManagerWidget для обработки двойного клика
                        self.file_manager_widget._on_table_double_clicked(current_idx)
                    # В любом случае принимаем событие, чтобы окно не закрылось по Энтер
                    event.accept()
                    return
...
class ImagePreviewDialog(QDialog):
    ...
    def __init__(self, current_index, image_list, current_language, parent=None):
        ...
        if parent:
            parent_geom = parent.window().geometry()
            w = parent_geom.width()
            h = parent_geom.height()
            new_h = int(h * 1.2)
            
            # Center vertically relative to parent
            new_y = parent_geom.y() - (new_h - h) // 2
            self.setGeometry(parent_geom.x(), new_y, w, new_h)
        else:
            self.showMaximized()
            
        # Сохраняем начальную геометрию для корректного восстановления
    ...
    def load_image(self):
        ...
        if self.pixmap.isNull():
            self.view_area.setText("Error loading image")
        else:
    ...
    def mouseReleaseEvent(self, event):
        if hasattr(self, 'dragging') and self.dragging:
            self.dragging = False
            # Если окно не развернуто, запоминаем новую позицию как "нормальную"
    ...
    def eventFilter(self, obj, event):
        """Перехват двойного клика по области просмотра и самой картинке."""
    ...
    def _on_replace(self):
        ...
        # Запрашиваем актуальные данные (имя файла могло измениться, напр. .jpg -> .png)
