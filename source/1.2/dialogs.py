...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QComboBox, QProgressBar, QGridLayout, QWidget, QPlainTextEdit, QTextBrowser, QSlider, QStyle, QFileDialog, QFrame, QLineEdit, QLayout, QSizePolicy, QScrollArea, QSizeGrip, QMessageBox, QApplication, QAbstractScrollArea
...
from widgets import ToggleSwitch, ZoomablePreviewArea
...
class CustomDialog(QDialog):
    ...
    def mousePressEvent(self, event):
        """Обработка нажатия мыши для перетаскивания или ресайза (ЛКМ и ПКМ)"""
...
class AudioPlayerDialog(CustomDialog):
    ...
    def setup_ui(self):
        ...
        self.info_label.setStyleSheet("color: #aaaaaa; font-size: 11px; margin-top: -5px; padding-right: 45px;")
    ...
    def set_volume(self, value):
        ...
        if self.parent() and hasattr(self.parent(), 'set_audio_volume'):
            ...
            # Fallback если вдруг родитель не тот
            vol = value / 100.0
    ...
    def on_slider_released(self):
        ...
        try:
            ...
            self.play_pause_btn.setText("\u23F8\uFE0E")
            self.play_pause_btn.setStyleSheet(self.btn_style_base.format(size=26, top=self.play_btn_top_offset))
            # self.play_pause_btn.setToolTip(get_translation(self.current_language, 'pause_btn')) # Removed tooltip
        except Exception as e:
...
class AIContextWindow(CustomDialog):
    ...
    def setup_ui(self):
        ...
        self.separator.setStyleSheet("background-color: #555555; border: none;")
...
class FilesWindow(CustomDialog):
    ...
    def __init__(self, current_language, parent=None):
        ...
        self.file_manager_widget = FileManagerWidget(current_language, self)
...
class ImagePreviewDialog(CustomDialog):
    ...
    def mousePressEvent(self, event):
        ...
        # 3. Перетаскивание окна (ЛКМ или ПКМ)
        if event.button() in (Qt.LeftButton, Qt.RightButton):
...
class TypeMismatchDialog(CustomDialog):
    ...
    def __init__(self, current_language, original_ext, new_ext, parent=None):
        ...
        # Определяем категории для текста
        ...
        title_label.setAlignment(Qt.AlignCenter)
        title_label.setStyleSheet("color: #ff3333; font-weight: bold; font-size: 16px; background-color: transparent;")
        layout.addWidget(title_label)

        # Выводим сообщение о том, что заменяется на что
        ...
        desc_label.setStyleSheet("color: #dddddd; font-size: 13px; background-color: transparent;")
        layout.addWidget(desc_label)
...
class StandardQuestionDialog(CustomDialog):
    ...
    def __init__(self, title, message, current_language, parent=None, yes_text=None, no_text=None):
        ...
        self.msg_label.setStyleSheet("color: #ffffff; font-size: 14px; background: transparent;")
...
class StandardInfoDialog(CustomDialog):
    ...
    def __init__(self, title, message, current_language, parent=None, is_error=False):
        ...
        self.msg_label.setStyleSheet(f"color: {text_color}; font-size: 14px; background: transparent;")
        self.msg_label.setWordWrap(True)
        self.msg_label.setAlignment(Qt.AlignCenter)
        layout.addWidget(self.msg_label)
...
class VoiceDownloadWorker(QThread):
    """Поток для скачивания языковой модели."""
    progress = pyqtSignal(int, int, int) # percent, current, total (dummy)
    finished = pyqtSignal(bool, str) # Успех, voice_id
...
class TTSPreviewDialog(CustomDialog):
    ...
    def __init__(self, text, engine, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Настройка Премиум Озвучки")
        ...
        self.settings = QSettings("DCS_Translator", "Settings")
        ...
        # Заголовок
        title = QLabel("Настройка Озвучки Piper")
        title.setStyleSheet("color: #ff9900; font-size: 16px; font-weight: bold; background: transparent;")
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)
        
        # --- Блок выбора языка и голоса ---
        lang_voice_layout = QGridLayout()
        lang_voice_layout.setSpacing(10)
        
        lbl_lang = QLabel("Язык:")
        lbl_lang.setStyleSheet("color: #aaaaaa; font-size: 13px;")
        self.combo_lang = QComboBox()
        self.combo_lang.setCursor(Qt.PointingHandCursor)
        
        lbl_voice = QLabel("Голос:")
        lbl_voice.setStyleSheet("color: #aaaaaa; font-size: 13px;")
        ...
        combo_style = """
            ...
            QComboBox::down-arrow { image: none; } /* Можно добавить иконку */
            QComboBox QAbstractItemView {
                background-color: #2b2b2b;
                color: #ffffff;
                selection-background-color: #ff9900;
        ...
        self.combo_voice.setStyleSheet(combo_style)
        
        self.lbl_voice_status = QLabel("")
        self.lbl_voice_status.setFixedWidth(24) # Под иконку или галочку
        self.lbl_voice_status.setAlignment(Qt.AlignCenter)
        
        lang_voice_layout.addWidget(lbl_lang, 0, 0)
        lang_voice_layout.addWidget(self.combo_lang, 0, 1)
        lang_voice_layout.addWidget(lbl_voice, 0, 2)
        lang_voice_layout.addWidget(self.combo_voice, 0, 3)
        lang_voice_layout.addWidget(self.lbl_voice_status, 0, 4)
        lang_voice_layout.setColumnStretch(1, 1)
        lang_voice_layout.setColumnStretch(3, 2)
        
        layout.addLayout(lang_voice_layout)
        
        # Заполнение языков
        lang_names = {
            'ru': 'Русский', 'en': 'Английский', 'de': 'Немецкий', 
            'fr': 'Французский', 'es': 'Испанский', 'it': 'Итальянский', 
            'zh': 'Китайский', 'pl': 'Польский', 'uk': 'Украинский'
        }
        for lang_code in self.registry.keys():
            self.combo_lang.addItem(lang_names.get(lang_code, lang_code.upper()), lang_code)
        ...
        # Прогресс загрузки голоса (скрыт по умолчанию)
        ...
        # Кнопки генерации ИЛИ скачивания + Прослушивание
        action_layout = QHBoxLayout()
        self.btn_main_action = QPushButton("Сгенерировать превью")
        ...
        self.btn_play_preview = QPushButton("▶ Прослушать превью")
        ...
        self.btn_play_preview.setStyleSheet("""
            QPushButton { background-color: #2b2b2b; color: #ff9900; border: 1px solid #ff9900; padding: 10px; border-radius: 18px; font-weight: bold; font-size: 14px;}
            QPushButton:hover { background-color: #333333; }
            QPushButton:disabled { background-color: #222; color: #555; border-color: #555; }
        """)
        self.btn_play_preview.clicked.connect(self._play_preview)
        
        action_layout.addWidget(self.btn_main_action)
        action_layout.addWidget(self.btn_play_preview)
        ...
        # 1. Скорость
        grid.addWidget(QLabel("Скорость речи:"), 0, 0)
        self.slider_speed = QSlider(Qt.Horizontal)
        self.slider_speed.setRange(50, 200) # 0.5 - 2.0
        self.slider_speed.setValue(100)
        self.lbl_speed = QLabel("1.0x")
        self.slider_speed.valueChanged.connect(lambda v: self.lbl_speed.setText(f"{v/100:.1f}x"))
        grid.addWidget(self.slider_speed, 0, 1)
        grid.addWidget(self.lbl_speed, 0, 2)
        
        # 2. Паузы
        grid.addWidget(QLabel("Паузы (сек):"), 1, 0)
        self.slider_silence = QSlider(Qt.Horizontal)
        self.slider_silence.setRange(0, 100) # 0.0 - 1.0
        self.slider_silence.setValue(20)
        self.lbl_silence = QLabel("0.2s")
        self.slider_silence.valueChanged.connect(lambda v: self.lbl_silence.setText(f"{v/100:.1f}s"))
        grid.addWidget(self.slider_silence, 1, 1)
        grid.addWidget(self.lbl_silence, 1, 2)
        
        # 3. Вариативность
        grid.addWidget(QLabel("Интонация (Шум):"), 2, 0)
        self.slider_noise = QSlider(Qt.Horizontal)
        self.slider_noise.setRange(0, 100)
        self.slider_noise.setValue(67)
        self.lbl_noise = QLabel("0.67")
        self.slider_noise.valueChanged.connect(lambda v: self.lbl_noise.setText(f"{v/100:.2f}"))
        grid.addWidget(self.slider_noise, 2, 1)
        grid.addWidget(self.lbl_noise, 2, 2)
        ...
        # Кнопки Ок/Отмена (Пилюли) #1a1a1a
        btn_layout = QHBoxLayout()
        self.btn_cancel = QPushButton("Отмена")
        self.btn_cancel.setCursor(Qt.PointingHandCursor)
        self.btn_cancel.setStyleSheet("""
            QPushButton { background-color: #444; color: white; border: none; padding: 10px 30px; border-radius: 18px; font-weight: bold; }
            QPushButton:hover { background-color: #555; }
        """)
        self.btn_cancel.clicked.connect(self.reject)
        
        self.btn_apply = QPushButton("Заменить в миссии")
        self.btn_apply.setCursor(Qt.PointingHandCursor)
        self.btn_apply.setEnabled(False) # Только после успешной генерации
        self.btn_apply.setStyleSheet("""
            QPushButton { background-color: #ff9900; color: #1a1a1a; border: none; padding: 10px 30px; border-radius: 18px; font-weight: bold; }
        ...
        lang_idx = self.combo_lang.findData(saved_lang)
        
        if lang_idx >= 0 and self.combo_lang.currentIndex() != lang_idx:
            self.combo_lang.setCurrentIndex(lang_idx)
        else:
            self._on_lang_changed() # Триггер заполнения голосов, если индекс не менялся

    def _get_main_btn_style(self, is_download=False):
        if is_download:
            return """
                QPushButton { background-color: transparent; color: #ff9900; border: 2px solid #ff9900; padding: 10px; border-radius: 18px; font-weight: bold; font-size: 14px;}
                QPushButton:hover { background-color: rgba(255, 153, 0, 0.1); }
            """
        else:
            return """
                QPushButton { background-color: #444; color: white; border: none; padding: 10px; border-radius: 18px; font-weight: bold; font-size: 14px;}
                QPushButton:hover { background-color: #555; }
    ...
    def _on_lang_changed(self):
        ...
        # Пытаемся восстановить прошлый голос для этого языка
        saved_voice = self.settings.value(f"tts_last_voice_{lang_code}")
        if saved_voice:
            v_idx = self.combo_voice.findData(saved_voice)
            if v_idx >= 0:
                self.combo_voice.setCurrentIndex(v_idx)
    ...
    def _on_voice_changed(self):
        ...
        lang_code = self.combo_lang.currentData()
        self.settings.setValue(f"tts_last_voice_{lang_code}", voice_id)
        
        installed = self.engine.get_installed_voices()
        is_installed = voice_id in installed
        
        if is_installed:
            self.lbl_voice_status.setText("✅")
            self.lbl_voice_status.setStyleSheet("color: #00cc66; font-size: 16px;")
            self.btn_main_action.setText("⚙️ Сгенерировать превью")
            self.btn_main_action.setStyleSheet(self._get_main_btn_style(is_download=False))
            self.btn_play_preview.show()
        else:
            self.lbl_voice_status.setText("📥")
            self.lbl_voice_status.setStyleSheet("color: #ffaa00; font-size: 16px;")
            # Имя голоса
            v_name = self.combo_voice.currentText()
            self.btn_main_action.setText(f"📥 Скачать модель: {v_name} (~20MB)")
            self.btn_main_action.setStyleSheet(self._get_main_btn_style(is_download=True))
    ...
    def _on_main_action_clicked(self):
        ...
        if voice_id not in installed:
            self._start_download(voice_id)
        else:
            self._generate_preview()

    def _start_download(self, voice_id):
        self.btn_main_action.setEnabled(False)
        self.btn_main_action.setText("Скачивание... Пожалуйста, подождите")
        self.download_progress.show()
        self.download_progress.setValue(0)
        self.lbl_status.setText(f"Скачивание модели {voice_id}...")
    ...
    def _on_download_finished(self, success, voice_id):
        self.download_progress.hide()
        self.btn_main_action.setEnabled(True)
        if success:
            self.lbl_status.setText("✅ Модель скачана! Можете генерировать.")
            self._on_voice_changed() # Обновит UI на кнопку "Генерация"
        else:
            self.lbl_status.setText("❌ Ошибка скачивания модели. Проверьте сеть.")
            self.btn_main_action.setText("Повторить скачивание")

    def _generate_preview(self):
        """Запускает генерацию с текущими параметрами и выбранным голосом."""
        ...
        self.preview_path = os.path.join(tempfile.gettempdir(), f"preview_{random.randint(1000,9999)}.wav")
        
        kwargs = {
            'length_scale': self.slider_speed.value() / 100.0,
            'sentence_silence': self.slider_silence.value() / 100.0,
            'noise_scale': self.slider_noise.value() / 100.0
        }
        
        success = self.engine.generate_speech(self.text, self.preview_path, voice_id=voice_id, **kwargs)
        
        self.btn_main_action.setEnabled(True)
        if success and os.path.exists(self.preview_path):
            self.lbl_status.setText("✅ Готово. Нажмите «Прослушать»")
    ...
    def _play_preview(self):
        if not self.preview_path or not os.path.exists(self.preview_path): return
        try:
            pygame.mixer.music.load(self.preview_path)
            pygame.mixer.music.play()
            self.is_playing = True
        except Exception as e:
            self.lbl_status.setText(f"Ошибка воспроизведения: {e}")
