...
class SettingsWindow(CustomDialog):
    ...
    def __init__(self, current_language, parent=None):
        ...
        line_bookmarks.setStyleSheet("background-color: #555555; border: none;")
    ...
    def load_values_from_parent(self):
        if self.parent_window:
            ...
            c_bm_star = getattr(self.parent_window, 'bookmark_bg_star', '#967800')
            c_bm_question = getattr(self.parent_window, 'bookmark_bg_question', '#0f5000')
            c_bm_alert = getattr(self.parent_window, 'bookmark_bg_alert', '#5a0000')
            op_bm = getattr(self.parent_window, 'bookmark_bg_opacity', 0.70)
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            # Reset bookmarks
            self.bookmark_star_preview.setStyleSheet('background-color: #967800; border: 1px solid #777;')
            self.bookmark_question_preview.setStyleSheet('background-color: #0f5000; border: 1px solid #777;')
            self.bookmark_alert_preview.setStyleSheet('background-color: #5a0000; border: 1px solid #777;')
            self.bookmark_opacity_slider.setValue(30)
            self.bookmark_opacity_val_label.setText("30%")
    ...
    def save_and_close(self):
        ...
        if self.parent_window:
            ...
            self.parent_window.bookmark_bg_star = m_bm_star.group(1).strip() if m_bm_star else '#967800'
            
            m_bm_question = re.search(r'background-color:\s*([^;]+);', self.bookmark_question_preview.styleSheet())
            self.parent_window.bookmark_bg_question = m_bm_question.group(1).strip() if m_bm_question else '#0f5000'
            
            m_bm_alert = re.search(r'background-color:\s*([^;]+);', self.bookmark_alert_preview.styleSheet())
            self.parent_window.bookmark_bg_alert = m_bm_alert.group(1).strip() if m_bm_alert else '#5a0000'
...
class TTSPreviewDialog(CustomDialog):
    ...
    def __init__(self, text, engine, parent=None, shared_tts_duration_cache=None):
        ...
        self.cb_replace_symbols.setChecked(False)
        ...
        self.btn_cancel = QPushButton(get_translation(self.current_language, 'cancel_btn'))
    ...
    def update_ui_texts(self):
        ...
        self.btn_cancel.setText(get_translation(self.current_language, 'cancel_btn'))
    ...
    def _load_settings(self):
        ...
        # Замена символов
        rep_enabled = self.settings.value("tts_replace_symbols_enabled", False)
    ...
    def _on_apply_checked_clicked(self):
        ...
        if replaced > 0:
            ...
            # Обновляем только заменённые строки (не перестраивая всю таблицу)
            all_files = self.miz_resource_manager.get_all_resource_files()
