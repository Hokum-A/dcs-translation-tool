...
class MizResourceManager:
    ...
    def parse_mission_image_links(self):
        ...
        for suffix, target_list_name in [('B', 'image_briefing_blue'), 
            ...
            if match:
                block = match.group(1)
                # Извлекаем все ResKey из блока
                keys = re.findall(r'\[\d+\]\s*=\s*"(ResKey_[^"]+)"', block)
                setattr(self, target_list_name, keys)
    ...
    def replace_audio(self, key, source_path):
        ...
        try:
            ...
            temp_name = f"dcs_repl_{base}_{suffix}{ext}"
    ...
    def rename_resource(self, res_key, new_filename, miz_path):
        """Переименовывает ресурс в ТЕКУЩЕЙ локали.
        
        Обновляет mapResource и переносит данные в pending_files.
        """
        ...
        if not is_current:
            # Если файл из DEFAULT, мы не можем его просто переименовать (это сломает другие локали).
            # Поэтому мы фактически делаем 'replace' самого себя, но с новым именем в RU.
            # Извлекаем оригинал во временный файл и регистрируем как новый.
            temp_path = self.extract_audio_to_temp(miz_path, res_key)
            if not temp_path:
                return False
            ...
            if old_path_in_zip in self.pending_files:
                # Если файл уже был заменен/переименован и ждет сохранения
                stored_source = self.pending_files.pop(old_path_in_zip)
                self.pending_files[new_path_in_zip] = stored_source
            else:
                # Файл только в ZIP. Нужно извлечь и переложить в pending.
                temp_path = self.extract_audio_to_temp(miz_path, res_key)
                if not temp_path:
                    return False
                self.pending_files[new_path_in_zip] = temp_path
                # Помечаем старый на удаление из архива (если больше не используется)
    ...
    def is_audio_replaced(self, dict_key):
        ...
        # Проверяем только отложенные (ещё не сохранённые) замены
