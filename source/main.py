...
v1.1 - Добавлены: обработка слешей, произвольные фильтры, сохранение настроек, система маркеров
v1.2 - Добавлены: нумерация строк, кастомные скроллбары, логотип
v1.3 - Обновлен интерфейс: тёмная тема, toggle-переключатели, hover-эффекты кнопок
v1.0 - Добавлена поддержка .miz файлов миссий DCS, исправлена замена файлов в архивах
v1.4 - Полностью переработан парсер для работы с файлами dictionary как с обычными текстовыми файлами

=== ВАЖНЫЕ ПРИНЦИПЫ ===
• Обратная совместиность: не ломать существующие функции
...
class TranslationApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.ai_context_1 = get_translation(self.current_language, 'default_context_text')     # Основной контекст ИИ (дефолт из локализации)
    ...
    def setup_top_panel(self, main_layout):
        ...
        self.open_btn = QPushButton('📂 Открыть файл (dictionary, .txt)')
        ...
        # ЦЕНТРАЛЬНАЯ ПАНЕЛЬ: заголовок "DCS Translation TOOL v1.0" - АБСОЛЮТНОЕ ПОЗИЦИОНИРОВАНИЕ
        ...
        # "v1.0" - выровнен по нижней линии
        version_title = QLabel('v1.0')
    ...
    def save_ai_context_settings(self, context_1, context_2):
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.settings_file):
            try:
                ...
                self.ai_context_1 = settings.get('ai_context_1', get_translation(self.current_language, 'default_context_text'))
    ...
    def open_file(self):
        ...
        try:
            ...
            file_path, _ = QFileDialog.getOpenFileName(
                self, 'Открыть файл', start_folder, 'Текстовые файлы (*.txt);;Lua файлы (*.lua);;Все файлы (*)')
            ...
            try:
                ...
                # Проверяем, является ли это dictionary файлом
                if '["' in self.original_content and '"] = "' in self.original_content:
        ...
        try:
            ...
            for i, line_data in enumerate(self.original_lines):
                ...
                if has_slash_mismatch:
                    mismatch_count += 1
                    mismatch_lines.append(i + 1)
                
                # Подсвечиваем строки со слешами
                if line_data.get('ends_with_backslash', False):
                ...
                # Индикатор несовпадения слешей
                if has_slash_mismatch:
                    warning_icon_label.setText("⚠")
                    warning_icon_label.setStyleSheet('''
                        color: #ff9900; 
                        font-weight: bold; 
                        font-size: 12px;
                        background-color: transparent;
    ...
    def show_save_report(self, save_path, missing_slashes=None, backup_path=None):
        ...
        # Заголовок
        title_label = QLabel(get_translation(self.current_language, 'file_save_success', filename=os.path.basename(save_path)))
        ...
        if missing_slashes:
            stats_text += f"\n\n{get_translation(self.current_language, 'slash_warning', count=len(missing_slashes))}"
            
        if backup_path:
            stats_text += f"\n\n{get_translation(self.current_language, 'backup_created')}\n{os.path.basename(backup_path)}"
    ...
    def show_miz_save_dialog(self):
        ...
        # Имя файла (оранжевым цветом)
        filename_label = QLabel(os.path.basename(self.current_miz_path))
        ...
        btns_layout.addWidget(save_txt_btn)
    ...
    def handle_miz_save(self, dialog, action):
        ...
        dialog.accept()
        if action == 'overwrite':
            self.save_miz_overwrite()
        elif action == 'save_as':
            # Сначала спрашиваем папку
            save_dialog = MizSaveAsDialog(self.current_language, self)
            if save_dialog.exec_() == QDialog.Accepted:
                self.save_miz_as(save_dialog.selected_folder)

    def save_miz_overwrite(self):
        """Перезаписывает исходный .miz файл"""
        progress = MizProgressDialog(self)
        try:
            print(f"\n{'='*50}")
            ...
            if self.create_backup:
                ...
                backup_path = self.current_miz_path + '.backup'
                shutil.copy2(self.current_miz_path, backup_path)
                ...
                with zipfile.ZipFile(self.current_miz_path, 'r') as zin:
                    with zipfile.ZipFile(temp_miz, 'w', compression=zin.compressionlevel if hasattr(zin, 'compressionlevel') else zipfile.ZIP_DEFLATED) as zout:
                        ...
                        for item in zin.infolist():
                            if item.filename == f'l10n/{self.current_miz_folder}/dictionary':
