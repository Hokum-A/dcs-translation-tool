...
from PyQt5.QtWidgets import QDialog, QVBoxLayout, QHBoxLayout, QPushButton, QLabel, QComboBox, QProgressBar, QGridLayout, QWidget, QPlainTextEdit, QTextBrowser, QSlider, QStyle, QFileDialog, QFrame, QLineEdit, QLayout, QSizePolicy
...
class SettingsWindow(CustomDialog):
    ...
    def __init__(self, current_language, parent=None):
        ...
        # поднять настройки выше: уменьшен верхний отступ, нижний отступ 15px по требованию
        content_layout.setContentsMargins(20, 0, 20, 15)
        ...
        # Action buttons: Cancel (white) and Save (orange)
    ...
    def load_values_from_parent(self):
        ...
        # log loaded values
        try:
            self._log_debug(f"load_values_from_parent: LOADED from parent: highlight={val}, color={col}, logs={logs} || SAVED snapshot: {self._orig_parent_values}")
        except Exception as e:
            self._log_debug(f"load_values_from_parent: ERROR: {e}")
    ...
    def reset_to_defaults(self):
        ...
        try:
            ...
            self.color_preview.setStyleSheet('background-color: #555555; border: 1px solid #777;')
            try:
    ...
    def open_color_dialog(self):
        ...
        if qc.exec_():
            ...
            try:
                self._log_debug(f'color selected: {c}')
            except Exception:
                pass
    ...
    def save_and_close(self):
        ...
        if self.parent_window:
            ...
            try:
                ...
                pass
            try:
                self.parent_window.save_settings(force=True)
            except TypeError:
