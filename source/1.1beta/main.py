...
"""
        ...
        try:
            for group_indices in batch:
                ...
                for idx in group_indices:
                    ...
                    # Цвет текста: theme_text_modified если изменен (и не сохранен), иначе theme_text_saved
    ...
    def show_miz_save_dialog(self):
        ...
        try:
            ...
            # Исправлено: обращаемся к QDialog.Rejected (глобально), а не к dialog.Rejected
    ...
    def on_preview_line_inserted(self, after_index, move_text, trans_edit_widget,
        ...
        try:
            ...
            new_orig = PreviewTextEdit(new_index, '', read_only=True, parent=self)
            ...
            try:
                ...
                # СИНХРОНИЗАЦИЯ РЕФЕРЕНСА: обновляем текст всех оригинальных виджетов группы
                self._sync_group_reference_widgets(key)
            except Exception:
                pass
    ...
    def _sync_group_reference_widgets(self, key):
        ...
        try:
            ...
            from widgets import PreviewTextEdit
            for edit in ow.findChildren(PreviewTextEdit):
                # Текст референса зависит от part_index (индекс строки внутри ключа)
                ...
                if ref_parts:
                    ...
                    # Fallback к display_text если данных нет (только для .miz)
                    if getattr(self, 'current_miz_path', None):
                        # Находим соответствующую строку в original_lines по индексу виджета
