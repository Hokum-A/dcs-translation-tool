...
"""
        ...
        # Настройки подсветки пустых полей (по умолчанию - ВКЛ и цвет #555555)
        self.highlight_empty_fields = True
        self.highlight_empty_color = '#555555'
    ...
    def load_settings(self):
        ...
        if os.path.exists(self.settings_file):
            try:
                ...
                self.highlight_empty_color = settings.get('highlight_empty_color', '#555555')
        ...
        try:
            ...
            settings = {
                ...
                'filter_empty_keys': self.filter_empty_keys_cb.isChecked() if hasattr(self, 'filter_empty_keys_cb') else getattr(self, 'filter_empty_keys', True),
                    ...
                    'highlight_empty_color': getattr(self, 'highlight_empty_color', '#5d7186'),
        ...
        try:
            ...
            try:
                ...
                if self.original_lines:
                    ...
                    # Инициализируем original_translated_text для отслеживания правок
        ...
        try:
            ...
            for key, (text_parts, file_lines, absolute_start_line) in entries.items():
                ...
                for part_index, part in enumerate(text_parts):
                    ...
                    include_in_original = should_translate and not (is_empty and self.filter_empty)
    ...
    def update_display(self):
        ...
        try:
            ...
            if not getattr(self, 'is_updating_from_preview', False):
    ...
    def update_preview(self):
        ...
        if PreviewTextEdit and isinstance(focused, PreviewTextEdit):
            # postpone until user finishes editing
            self.schedule_preview_update(300)
            return
    ...
    def render_preview_batch(self):
        ...
        try:
            for group_indices in batch:
                ...
                bg_color = "#505050" if current_count % 2 == 0 else "#4a4a4a"
                ...
                for idx in group_indices:
                    ...
                    orig_edit = PreviewTextEdit(idx, l_data['display_text'], read_only=True)
                    ...
                    trans_edit.line_inserted.connect(
                        lambda ins_idx, te=trans_edit,
                               orl=orig_row_layout, trl=trans_row_layout,
                               mw=meta_row_widget, orw=orig_row_widget, trw=trans_row_widget:
                        self.on_preview_line_inserted(ins_idx, te, orl, trl, mw, orw, trw)
                    )
                    # Подключаем удаление строки через Backspace
                    trans_edit.line_deleted.connect(lambda del_idx, te=trans_edit: self.on_preview_line_deleted(del_idx, te))
    ...
    def generate_translated_content(self):
        ...
        try:
            ...
            for line_data in self.all_lines_data:
                ...
                # Если translated_text содержит \n (пользователь добавил строки через Enter),
                # сплитим его на части — каждая часть станет отдельной Lua-строкой с \\ разделителем
                raw_translated = line_data['translated_text'] if line_data['translated_text'] is not None else ''
                orig_text = line_data['original_text'] if line_data['original_text'] else ''
                if raw_translated == '' and orig_text == '':
                    # Пустая строка (добавлена пользователем) — сохраняем как пустую часть
                    translations[key].append('')
                elif raw_translated:
                    parts = raw_translated.split('\n')
                    translations[key].extend(parts)
                else:
                    translations[key].append(orig_text)
    ...
    def reset_modified_display_state(self):
        ...
        try:
            ...
            # Обновляем левое окно (Оригинал)
            if hasattr(self, 'original_text_all'):
                new_orig_lines = [line['original_text'] for line in self.original_lines]
                v_pos = self.original_text_all.verticalScrollBar().value()
                self.original_text_all.setPlainText('\n'.join(new_orig_lines))
                self.original_text_all.verticalScrollBar().setValue(v_pos)
    ...
    def on_preview_line_inserted(self, after_index, trans_edit_widget,
        ...
        try:
            ...
            # Перевод: пустая редактируемая строка
            new_trans = PreviewTextEdit(new_index, '', read_only=False)
            new_trans_style = "color: #2ecc71; background-color: transparent; border: none; border-radius: 0px;"
            ...
            new_trans.line_inserted.connect(
                lambda ins_idx, te=new_trans,
                       orl=orig_row_layout, trl=trans_row_layout,
                       mw=meta_row_widget, orw=orig_row_widget, trw=trans_row_widget:
                self.on_preview_line_inserted(ins_idx, te, orl, trl, mw, orw, trw)
            )
            # Подключаем удаление строки для динамически вставленной строки
            new_trans.line_deleted.connect(lambda del_idx, te=new_trans: self.on_preview_line_deleted(del_idx, te))
            ...
            self._insert_line_in_upper_editor(new_index)
    ...
    def _insert_line_in_upper_editor(self, new_index):
        ...
        try:
            ...
            try:
                ...
                for attr in ('original_text_all', 'translated_text_all'):
                    ...
                    block = doc.findBlockByNumber(insert_at)
                    if block.isValid():
                    ...
                    # Вставляем пустой блок (новую строку)
                    cursor.insertBlock()
                    cursor.endEditBlock()
            finally:
    ...
    def _delete_line_from_upper_editor(self, index):
        ...
        editors = []
        if hasattr(self, 'original_text_all'):
            editors.append(self.original_text_all)
        if hasattr(self, 'translated_text_all'):
            editors.append(self.translated_text_all)
    ...
    def on_preview_line_deleted(self, index, trans_edit_widget):
    ...
    def update_preview_for_key(self, key):
        ...
        try:
            ...
            ErrorLogger.log_error("PREVIEW_UPDATE_KEY", f"Error in update_preview_for_key: {e}")
            raise # Передаем выше для срабатывания fallback если совсем всё плохо
