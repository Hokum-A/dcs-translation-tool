...
"""
    ...
    def handle_new_instance_connection(self):
        ...
        # Эта функция вызывается в старом окне, когда новая копия посылает ей путь файла
        # Сервер читается через атрибут QLocalServer
        ...
        if socket:
            if socket.waitForReadyRead(1000):
                ...
                # Жёсткий трюк для Windows: поднимаем окно поверх всех окон, затем снимаем флаг,
                # чтобы обойти защиту Windows от перехвата фокуса (Anti-Focus-Stealing)
                self.setWindowFlag(Qt.WindowStaysOnTopHint, True)
                self.show()
                self.setWindowFlag(Qt.WindowStaysOnTopHint, False)
                self.show()
                
                # Гарантированно забираем фокус ввода
                self.activateWindow()
                self.raise_()
...
if __name__ == "__main__":
    ...
    if socket.waitForConnected(500):
        ...
        socket.disconnectFromServer()
        sys.exit(0) # Моментально закрываем эту "лишнюю" копию
        
    # Если мы здесь, значит это ПЕРВЫЙ запуск программы.
