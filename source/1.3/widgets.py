...
from PyQt5.QtWidgets import (
    QWidget, QPlainTextEdit, QScrollBar, QLineEdit, QCheckBox, QLabel, 
    QHBoxLayout, QFrame, QVBoxLayout, QScrollArea, QSizePolicy, QMenu, 
    QSlider, QHeaderView, QPushButton, QStyle, QProxyStyle, 
    QSplitter, QSplitterHandle,
    QStyleOptionHeader, QStyleOptionSlider, QApplication, QTableWidgetItem
)
from PyQt5.QtCore import (
    Qt, pyqtSignal, QTimer, QPropertyAnimation, QRect, QEasingCurve, 
    QPoint, pyqtProperty, QSize, QObject, QEvent
)
from PyQt5.QtGui import (
    QFont, QColor, QPalette, QPainter, QBrush, QPainterPath, 
    QRegion, QTextCursor, QPixmap, QPen, QPolygon, QCursor, QFontMetrics
)
from PyQt5.QtCore import QRectF
from PyQt5 import sip

class CustomSplitterHandle(QSplitterHandle):
    """Кастомная ручка разделителя с плавным ховером и без лишних насечек."""
    def __init__(self, orientation, parent):
        super().__init__(orientation, parent)
        self._alpha = 0
        self.animation = QPropertyAnimation(self, b"hover_alpha")
        self.animation.setDuration(350) # Плавность 0.35 сек
        
        self.delay_timer = QTimer(self)
        self.delay_timer.setSingleShot(True)
        self.delay_timer.timeout.connect(self._start_fade_in)
    ...
    def paintEvent(self, event):
        # ВАЖНО: Мы перекрываем paintEvent полностью, не вызывая super().paintEvent()
        # Это гарантирует, что стандартные точки (grip) не будут нарисованы.
        painter = QPainter(self)
        # Рисуем либо прозрачный фон, либо оранжевый с текущей альфой
        color = QColor(255, 153, 0, self._alpha)
        painter.fillRect(self.rect(), color)
        painter.end()

    def enterEvent(self, event):
        self.delay_timer.start(500)
    ...
    def _start_fade_in(self):
        self.animation.stop()
        self.animation.setStartValue(self._alpha)
...
class CustomSplitter(QSplitter):
    """Сплиттер, использующий кастомные ручки."""
...
class NumberedTextEdit(QPlainTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        # Цвета для "зебры" по умолчанию
        self._zebra_even = QColor(80, 80, 80) # #505050
        self._zebra_odd = QColor(74, 74, 74)  # #4a4a4a
    ...
    def select_lines_at_y(self, y, extend=False):
        ...
        if not extend:
            ...
            # Устанавливаем якорь в конец текста, а позицию в начало (чтобы курсор мигал в начале)
            ...
            if current_num >= start_num:
                # Тянем вниз: якорь в начале стартового блока, позиция — конец текущего (без \n)
                cursor.setPosition(anchor_block.position(), QTextCursor.MoveAnchor)
                cursor.setPosition(current_block.position() + len(current_block.text()), QTextCursor.KeepAnchor)
            else:
                # Тянем вверх: якорь в конце стартового блока (без \n), позиция — начало текущего
                # Находим позицию конца текста в стартовом блоке
                anchor_end = anchor_block.position() + len(anchor_block.text())
                cursor.setPosition(anchor_end, QTextCursor.MoveAnchor)
                cursor.setPosition(current_block.position(), QTextCursor.KeepAnchor)

        # Сохраняем положение горизонтального скролла, чтобы избежать прыжков при выделении длинных строк
        h_scroll = self.horizontalScrollBar().value()
        self.setTextCursor(cursor)
        self.horizontalScrollBar().setValue(h_scroll)

    def set_zebra_colors(self, even_color, odd_color):
        """Обновляет цвета зебры (принимает QColor или строку)"""
        if isinstance(even_color, str):
            self._zebra_even = QColor(even_color)
        else:
            self._zebra_even = even_color
            
        if isinstance(odd_color, str):
            self._zebra_odd = QColor(odd_color)
        else:
            self._zebra_odd = odd_color
    ...
    def paintEvent(self, event):
        ...
        # Используем настраиваемые цвета для "зебры"
        color_even = self._zebra_even
        color_odd = self._zebra_odd
    ...
    def contextMenuEvent(self, event):
        ...
        # Добавляем пункт "Replace" если есть выделение
        cursor = self.textCursor()
        if cursor.hasSelection():
            ...
            p = self.parent()
            while p:
                if hasattr(p, 'start_find_replace'):
                    main_window = p
                    break
                p = p.parent()
            ...
            if main_window:
                ...
                # В NumberedTextEdit (верхние окна) индекс строки = номеру блока
                pref_idx = cursor.blockNumber()
                
                # Определяем preferred_type. Верхнее левое - readOnly (Оригинал), верхнее правое - редактируемое (Перевод)
                pref_type = 'text_original' if self.isReadOnly() else 'text_translated'
                
                find_action = menu.addAction("Find")
                find_action.triggered.connect(lambda _=False, pi=pref_idx, pt=pref_type: main_window.start_find_replace(selected_text, focus_type='find', preferred_index=pi, preferred_type=pt))
                
                replace_action = menu.addAction("Replace")
                replace_action.triggered.connect(lambda _=False, pi=pref_idx, pt=pref_type: main_window.start_find_replace(selected_text, focus_type='replace', preferred_index=pi, preferred_type=pt))

        menu.exec_(event.globalPos())

    def _is_smart_paste_enabled(self):
        """Проверяет, включена ли умная вставка в настройках главного окна."""
        if getattr(self, 'disable_smart_paste', False):
            return False
        p = self.parent()
        while p:
            if hasattr(p, 'smart_paste_enabled'):
                return p.smart_paste_enabled
            p = p.parent()
        return True  # По умолчанию включена

    def keyPressEvent(self, event):
        """Перехват Ctrl+V: если буфер содержит маркеры 🔹N🔹, делегируем главному окну."""
        if event.key() == Qt.Key_V and event.modifiers() & Qt.ControlModifier:
            clipboard_text = QApplication.clipboard().text()
            import re
            if clipboard_text and re.search(r'🔹\d+🔹', clipboard_text) and self._is_smart_paste_enabled():
                # Ищем главное окно в иерархии родителей
                p = self.parent()
                while p:
                    if hasattr(p, 'handle_smart_paste_request'):
                        p.handle_smart_paste_request(clipboard_text)
                        return
                    p = p.parent()
        super().keyPressEvent(event)

    def insertFromMimeData(self, source):
        """Перехват вставки через контекстное меню (ПКМ -> Вставить) для маркеров."""
        import re
        if source.hasText() and re.search(r'🔹\d+🔹', source.text()) and self._is_smart_paste_enabled():
            p = self.parent()
            while p:
                if hasattr(p, 'handle_smart_paste_request'):
                    p.handle_smart_paste_request(source.text())
                    return
                p = p.parent()
...
class CustomToolTip(QLabel):
    ...
    def show_tooltip(self, text, widget, side='bottom'):
        ...
        try:
            ...
            rect = metrics.boundingRect(QRect(0, 0, 1100, 10000), Qt.TextWordWrap | Qt.AlignLeft, text)
            text_width = rect.width()
            text_height = rect.height()
            padding = self._padding
            
            # Базовые размеры
            new_w = text_width + padding * 2 + 10
            new_h = text_height + padding * 2
            
            # ОГРАНИЧЕНИЕ ШИРИНЫ (увеличено для длинных путей)
            max_tooltip_width = 1200
            effective_max_width = max_tooltip_width - padding * 4
            
            if text_width > effective_max_width:
                # Позволяем Qt самому переносить слова, задав фиксированную макс-ширину
                self.setMaximumWidth(max_tooltip_width)
                self.adjustSize()
                new_w = self.width()
                new_h = self.height()
            else:
                # Текст вмещается
                new_w = text_width + padding * 2 + 5
                new_h = text_height + padding * 2
            
            self._apply_positioning(text, new_w, new_h, widget.rect(), widget.mapToGlobal(QPoint(0,0)), side)

        except Exception as e:
            self._fallback_show(text, QPoint(0,0))

    def show_tooltip_at_rect(self, text, global_rect, side='bottom'):
        """Показывает тултип относительно глобального прямоугольника (для QAction и т.д.)"""
        if not text:
            self.hide()
            return
        ...
        try:
            ...
            # РАСЧЕТ ПОЗИЦИИ напрямую из глобального ректа
            x, y = 0, 0
            offset = 8
            
            if side == 'bottom':
                x = global_rect.x() + (global_rect.width() - new_w) // 2
                y = global_rect.y() + global_rect.height() + offset
            elif side == 'top':
                x = global_rect.x() + (global_rect.width() - new_w) // 2
                y = global_rect.y() - new_h - offset
            elif side == 'left':
                x = global_rect.x() - new_w - offset
                y = global_rect.y() + (global_rect.height() - new_h) // 2
            elif side == 'right':
                x = global_rect.x() + global_rect.width() + offset
                y = global_rect.y() + (global_rect.height() - new_h) // 2
    ...
    def show_tooltip_at_pos(self, text, global_pos, side='bottom'):
        """Показывает тултип в конкретных глобальных координатах (возле курсора)"""
        if not text:
            self.hide()
            return
        ...
        try:
            ...
            # РАСЧЕТ ПОЗИЦИИ (с небольшим смещением от курсора)
            offset = 15
    ...
    def _apply_positioning(self, text, new_w, new_h, target_rect, global_pos, side):
        """Внутренний метод для применения размеров и позиции"""
        new_w = max(30, new_w)
        new_h = max(20, new_h)
        self.setFixedSize(new_w, new_h)
    ...
    def _fallback_show(self, text, pos):
        """Запасной метод показа"""
        try:
            self.setText(text)
            metrics = self.fontMetrics()
            rect = metrics.boundingRect(QRect(0, 0, 1100, 10000), Qt.TextWordWrap | Qt.AlignLeft, text)
            ...
            text_height = rect.height()
            padding = self._padding
            new_w = text_width + padding * 2 + 5  # Уменьшил ширину
            new_h = text_height + padding * 2
            new_w = max(30, new_w)
            new_h = max(20, new_h)
            self.setFixedSize(new_w, new_h)
...
class JumpSlider(QSlider):
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            # Передаем управление базовому классу для корректного старта драг-н-дропа
            event.accept()
        super().mousePressEvent(event)
...
class ClickableLabel(QLabel):
    """QLabel, реагирующий на клики и Drag-and-Drop файлов."""
    clicked = pyqtSignal()
    ...
    def dropEvent(self, event):
        if event.mimeData().hasUrls():
            ...
            if urls:
                ...
                if file_path.lower().endswith(('.ogg', '.wav')):
...
class PreviewTextEdit(QPlainTextEdit):
    ...
    line_inserted = pyqtSignal(int, str)  # index строки, после которой нужно вставить новую, [move_text]
    line_deleted = pyqtSignal(int, str)   # index строки, которую нужно удалить, [merge_text]
    smart_paste_requested = pyqtSignal(str) # [SMART PASTE] Сигнал для глобальной вставки с маркерами
    ...
    def __init__(self, index, text, read_only=False, parent=None):
        ...
        self._last_emitted_text = text # Для предотвращения ложных срабатываний при переверстке
        ...
        # Шрифт (берем из главного окна или ставим Consolas 10 по умолчанию)
        font_family = "Consolas"
        font_size = 10
        if parent:
            # Ищем TranslationApp в иерархии родителей
            p = parent
            while p:
                if hasattr(p, 'preview_font_family'):
                    font_family = p.preview_font_family
                    font_size = p.preview_font_size
                    break
                p = p.parent()
        ...
        self.single_line = False # Если True, Enter и Backspace (удаление строки) не работают
    ...
    def insertFromMimeData(self, source):
        """Перехват вставки через контекстное меню (ПКМ -> Вставить)"""
        import re
        if not self.isReadOnly() and source.hasText() and re.search(r'🔹\d+🔹', source.text()) and self._is_smart_paste_enabled():
            self.smart_paste_requested.emit(source.text())
            return  # Блокируем стандартную вставку в курсор
        super().insertFromMimeData(source)
    
    def keyPressEvent(self, event):
        """Перехватываем Enter для вставки, Backspace для удаления, Ctrl+V для маркеров"""
        import re
        
        # [SMART PASTE] Перехват Ctrl+V для маркеров
        if not self.isReadOnly() and event.key() == Qt.Key_V and event.modifiers() & Qt.ControlModifier:
            clipboard_text = QApplication.clipboard().text()
            if clipboard_text and re.search(r'🔹\d+🔹', clipboard_text) and self._is_smart_paste_enabled():
                self.smart_paste_requested.emit(clipboard_text)
                return  # Блокируем стандартную вставку
                
        # 1. ENTER (Вставка новой)
        if not self.isReadOnly() and event.key() in (Qt.Key_Return, Qt.Key_Enter):
            if self.single_line:
                # В режиме одной строки Enter просто игнорируем или можно переключить фокус
                event.ignore()
                return
                
            try:
                # Разделяем текущий текст на левую и правую часть по курсору/выделению
                cursor = self.textCursor()
                ...
                if start < 0: start = 0
                if end < 0: end = 0
                if start > len(full): start = len(full)
                if end > len(full): end = len(full)

                before = full[:start]
                after = full[end:]

                # Устанавливаем левую часть в текущий редактор
                try:
                    self.blockSignals(True)
                    self.setPlainText(before)
                    self._last_emitted_text = before
                    # Помещаем курсор в конец левой части
                    c = self.textCursor()
                    c.setPosition(len(before))
                    self.setTextCursor(c)
                finally:
                    self.blockSignals(False)

                self.line_inserted.emit(self.index, after)
            except Exception:
                pass
            return
        ...
        if not self.isReadOnly() and event.key() == Qt.Key_Backspace:
            # Если есть выделение — позволяем стандартное удаление выделения
            # Если нет выделения и курсор в начале — наше спец. поведение (удаление/слияние)
            if not self.textCursor().hasSelection() and self.textCursor().atStart():
                if self.single_line:
                   # В режиме одной строки запрещаем удаление/слияние через Backspace в начале
                   super().keyPressEvent(event)
                   return

                try:
                    current = self.toPlainText()
                    # Если текущее поле пустое — обычное удаление строки
                    if current == "":
                        self.line_deleted.emit(self.index, '')
                    else:
                        # Иначе — запрашиваем объединение с предыдущей строкой (merge)
                        self.line_deleted.emit(self.index, current)
                except Exception:
                    pass
                return
    ...
    def focusInEvent(self, event):
        """Для виртуальных полей — убираем маркер при фокусе."""
        super().focusInEvent(event)
        if getattr(self, '_is_virtual', False):
            marker = getattr(self, '_virtual_marker', '')
            if marker and self.toPlainText().strip() == marker.strip():
                try:
                    self.blockSignals(True)
                    self.setPlainText("")
    ...
    def focusOutEvent(self, event):
        """Для виртуальных полей — если ничего не ввели, возвращаем маркер."""
        super().focusOutEvent(event)
        if getattr(self, '_is_virtual', False):
            marker = getattr(self, '_virtual_marker', '')
            if marker and not self.toPlainText().strip():
                try:
                    self.blockSignals(True)
                    self.setPlainText(marker)
                    self._last_emitted_text = marker
                    # Восстанавливаем стиль маркера
                    warn_style = "color: #999999; background-color: rgba(255, 165, 0, 20); border: none; border-radius: 0px;"
                    self.setStyleSheet(warn_style)
                    self._original_style = warn_style
                finally:
                    self.blockSignals(False)

    def mousePressEvent(self, event):
        """Клик мыши снимает extra-выделения (поиск) и подсветку аудио перед обычным поведением"""
        try:
            self.setExtraSelections([])
        except Exception:
            pass
        # [FIX] Снимаем оранжевую подсветку аудио при клике на текстовом поле
        try:
            main_win = self.window()
            if main_win and hasattr(main_win, 'clear_audio_highlight'):
                if getattr(main_win, 'highlighted_audio_key', None):
    ...
    def contextMenuEvent(self, event):
        ...
        try:
            ...
            # Добавляем наш пункт "Найти и заменить..." если есть выделение
            cursor = self.textCursor()
            if cursor.hasSelection():
                selected_text = cursor.selectedText().replace('\u2029', '\n')
                
                # Ищем главное окно в иерархии родителей по наличию метода start_find_replace
                main_window = None
                p = self.parent()
                while p:
                    if hasattr(p, 'start_find_replace'):
                ...
                if main_window:
                    ...
                    # Определяем preferred_index: для референсных полей (index=-1) ищем по ключу + part_index
                    pref_idx = self.index
                    if pref_idx is None or pref_idx < 0:
                        search_key = getattr(self, 'key', None)
                        search_part = getattr(self, 'part_index', 0)
                        if search_key and hasattr(main_window, 'original_lines'):
                            # Сначала пробуем найти точное совпадение по ключу И part_index
                            for li, ld in enumerate(main_window.original_lines):
                                if ld.get('key') == search_key and ld.get('part_index', 0) == search_part:
                                    pref_idx = li
                                    break
                            else:
                                # Fallback: первая строка с таким ключом
                                for li, ld in enumerate(main_window.original_lines):
                                    if ld.get('key') == search_key:
                                        pref_idx = li
                                        break
                    
                    # Определяем preferred_type на основе партнера или readOnly
                    pref_type = 'text_translated'
                    if self.isReadOnly():
                        if getattr(self, 'index', None) == -1:
    ...
    def on_content_changed(self):
        ...
        try:
            if not self.isReadOnly():
                current = self.toPlainText()
                if current != getattr(self, '_last_emitted_text', None):
                    self._last_emitted_text = current
                    self.text_changed.emit(self.index, current)
            QTimer.singleShot(0, self.adjust_height)
        except RuntimeError:
            pass
    ...
    def calculate_required_height(self):
        ...
        # Добавляем небольшой запас (4px) для исключения микро-скролла (сбалансированный вариант)
        h += 4
    ...
    def adjust_height(self):
        """Настройка высоты с синхронизацией row-siblings (контейнеры колонок).
        Каждый PreviewTextEdit устанавливает свою высоту по собственному содержимому,
        НЕ зависимо от партнёра. Контейнеры row_siblings выравниваются по максимуму."""
        if sip.isdeleted(self):
            return
    ...
    def enterEvent(self, event):
        ...
        try:
            ...
            if self.isReadOnly():
                return
            main_win = self.window()
            color = getattr(main_win, 'highlight_empty_color', '#434343')
    ...
    def leaveEvent(self, event):
        super().leaveEvent(event)
        try:
            # Если поле только для чтения (референс), мы не меняли его стиль в enterEvent,
            # поэтому не должны сбрасывать его здесь (чтобы не затирать цвет "Нет в референсе").
            if self.isReadOnly():
                return
...
class ZoomablePreviewArea(QScrollArea):
    """Область предпросмотра с поддержкой зума и панорамирования."""
    ...
    def __init__(self, parent=None):
        ...
        self.setFrameStyle(QFrame.NoFrame)
        self.setViewportMargins(0, 0, 0, 0)
        self.setStyleSheet("ZoomablePreviewArea { background: transparent; border: none; outline: none; }")
        
        # Кастомные скроллбары
        self.setVerticalScrollBar(CustomScrollBar())
        self.setHorizontalScrollBar(CustomScrollBar())
        
        # Внутренний QLabel для изображения
        self.image_label = QLabel()
        self.image_label.setAlignment(Qt.AlignCenter)
        self.image_label.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
        # Возвращаем обычную серую рамку (без скруглений) вокруг картинки
        self.image_label.setStyleSheet("background: transparent; border: 1px solid #555; border-radius: 0px;")
        ...
        # Переменные для панорамирования (драга)
        self._pan_active = False
        self._pan_start_pos = QPoint()
        
        # Для начальной подсказки / плейсхолдера
        self._is_placeholder = True

    def setPixmap(self, pixmap):
        """Устанавливает изображение и сбрасывает масштаб, подгоняя под окно."""
        ...
        # Подгоняем изображение под текущий размер области при первой загрузке
        # Вычитаем пару пикселей, чтобы гарантированно избежать появления скроллбаров
        view_size = self.viewport().size()
        target_w = max(1, view_size.width() - 2)
        target_h = max(1, view_size.height() - 2)
        ...
        if (pixmap.width() > target_w or pixmap.height() > target_h):
            ...
            # Устанавливаем стартовый зум так, чтобы картинка вписывалась
            self._zoom_factor = min(target_w / pixmap.width(), target_h / pixmap.height())
        else:
            scaled_pixmap = pixmap
            
        self.image_label.setPixmap(scaled_pixmap)
        # Учитываем 1px рамку с каждой стороны (итого +2px к размеру QLabel)
        self.image_label.resize(scaled_pixmap.width() + 2, scaled_pixmap.height() + 2)

    def wheelEvent(self, event):
        """Обработка зума колесиком мыши."""
        if self._is_placeholder or self._original_pixmap.isNull():
            return
        ...
        if event.angleDelta().y() > 0:
            ...
            if new_zoom > 10.0:  # Максимальный зум
                return
        else:
            # Зум OUT
            new_zoom = old_zoom / scale_factor
            # Минимальный зум - чтобы изображение не стало слишком мелким
            view_size = self.viewport().size()
            min_zoom = min(0.1, min(view_size.width() / self._original_pixmap.width(), 
                                    view_size.height() / self._original_pixmap.height()))
        ...
        # Запоминаем точку под курсором ДО зума
        old_pos = self.image_label.mapFrom(self.viewport(), event.pos())
        
        self._zoom_factor = new_zoom
        self._update_image_scale()
        
        # Рассчитываем новую позицию той же точки
        ratio = new_zoom / old_zoom
        new_pos_x = old_pos.x() * ratio
        new_pos_y = old_pos.y() * ratio
        
        # Корректируем скроллбары на разницу
        h_bar = self.horizontalScrollBar()
        v_bar = self.verticalScrollBar()
        h_bar.setValue(int(h_bar.value() + (new_pos_x - old_pos.x())))
        v_bar.setValue(int(v_bar.value() + (new_pos_y - old_pos.y())))

    def _update_image_scale(self):
        """Обновляет pixmap в QLabel на основе текущего zoom_factor."""
    ...
    def eventFilter(self, obj, event):
        """Перехватываем события мыши у image_label для панорамирования."""
        from PyQt5.QtCore import QEvent
        if not hasattr(self, 'image_label'):
            return super().eventFilter(obj, event)
            
        if obj == self.image_label:
            if event.type() == QEvent.MouseButtonPress and event.button() == Qt.MiddleButton and not self._is_placeholder:
                self._pan_active = True
                self._pan_start_pos = event.globalPos()
                self.viewport().setCursor(Qt.ClosedHandCursor)
                return True
            elif event.type() == QEvent.MouseMove and self._pan_active:
                delta = event.globalPos() - self._pan_start_pos
                ...
                self._pan_start_pos = event.globalPos()
                return True
            elif event.type() == QEvent.MouseButtonRelease and event.button() == Qt.MiddleButton and self._pan_active:
                self._pan_active = False
    ...
    def mousePressEvent(self, event):
        """Фолбэк если кликнули мимо картинки во вьюпорте."""
        if event.button() == Qt.MiddleButton and not self._is_placeholder:
            self._pan_active = True
            self._pan_start_pos = event.globalPos()
            self.viewport().setCursor(Qt.ClosedHandCursor)
            event.accept()
    ...
    def resizeEvent(self, event):
        """При изменении размера окна проверяем, нужно ли отцентровать мелкую картинку."""
        super().resizeEvent(event)
        # Если картинка меньше области, она центрируется автоматически благодаря setAlignment(Qt.AlignCenter)
...
class NaturalSortTableWidgetItem(QTableWidgetItem):
    """Кастомный элемент таблицы для естественной сортировки строк с числами."""
    def __lt__(self, other):
...
class FileManagerStyle(QProxyStyle):
    """Proxy-стиль для отрисовки элементов интерфейса в едином стиле."""

    def drawPrimitive(self, element, option, painter, widget=None):
        # 1. Отрисовка стрелок сортировки цветом #999999
        if element == QStyle.PE_IndicatorHeaderArrow:
            # Не рисуем стрелку для первой колонки, если это заголовок с чекбоксом (там она перекрывает чекбокс)
            if hasattr(option, 'section') and option.section == 0:
                if isinstance(widget, CheckboxHeader):
                    return
            ...
            # Отрисовка стрелки внутри предоставленного прямоугольника
            rect = option.rect
            ...
            if is_up:
                poly = QPolygon([QPoint(-size, size // 2),
                                 QPoint(size, size // 2),
                                 QPoint(0, -size)])
            else:
                poly = QPolygon([QPoint(-size, -size // 2),
        ...
        if element == QStyle.PE_IndicatorCheckBox:
            ...
            border_color = '#ff9900' if is_hover else '#666666'
    ...
    def pixelMetric(self, metric, option=None, widget=None):
        """Переопределяем размеры чекбоксов."""
        if metric == QStyle.PM_IndicatorWidth: return 19
        if metric == QStyle.PM_IndicatorHeight: return 19
        return super().pixelMetric(metric, option, widget)

    def subElementRect(self, subElement, option, widget):
        """Переопределяем положение стрелки сортировки и области текста."""
        if subElement == QStyle.SE_HeaderArrow:
            rect = option.rect
            arrow_size = 10  # Slightly smaller arrow
            # Помещаем стрелку максимально вправо (2px отступ)
            return QRect(rect.right() - arrow_size - 2, rect.center().y() - arrow_size // 2, arrow_size, arrow_size)
        
        if subElement == QStyle.SE_HeaderLabel:
            rect = option.rect
            # Резервируем 16 пикселей справа под стрелку, чтобы текст не налезал
            return QRect(rect.left(), rect.top(), rect.width() - 16, rect.height())
...
class CheckboxHeader(QHeaderView):
    ...
    def _get_checkbox_rect(self, section_rect):
        ...
        cb_size = 20
        # Смещение -2 (было -7) для смещения чекбокса на 5 пикселей вправо
        return QRect(
    ...
    def mousePressEvent(self, event):
        ...
        if logical_index == 0:
            ...
            if hitbox.contains(event.pos()):
                ...
                self.checkboxToggled.emit(self.check_state)
                self.updateSection(0)
                return
        
        # Для остальных колонок или если клик мимо чекбокса в первой колонке
        # (но мы блокируем супер-вызов для первой колонки полностью, чтобы убрать сортировку)
        if logical_index != 0:
            super().mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event):
        ...
        if is_hovered != getattr(self, 'is_hovered', False):
...
class ToolTipFilter(QObject):
    """Заменяет системные события ToolTip на CustomToolTip."""
...
class ButtonToolTipFilter(QObject):
    """Заменяет системные тултипы на QPushButton."""
...
class SearchPopup(QFrame):
    """Всплывающее окно настроек поиска (Option 1)"""
    def __init__(self, parent=None, current_language='ru'):
        super().__init__(parent)
        ...
        self.close_btn.setStyleSheet("background: transparent; color: #888; border: none; font-size: 16px; font-weight: bold;")
        ...
        # Checkboxes
        self.cb_orig = SearchCheckBox()
        self.cb_ref = SearchCheckBox()
        self.cb_edit = SearchCheckBox()
        self.cb_audio = SearchCheckBox()
        
        layout.addWidget(self.cb_orig)
        layout.addWidget(self.cb_ref)
        layout.addWidget(self.cb_edit)
    ...
    def retranslate_ui(self):
        ...
        self.title_label.setText(get_translation(self.current_language, 'search_scope_label'))
        self.cb_orig.setText(get_translation(self.current_language, 'scope_original'))
        self.cb_ref.setText(get_translation(self.current_language, 'scope_reference'))
        self.cb_edit.setText(get_translation(self.current_language, 'scope_editor'))
        self.cb_audio.setText(get_translation(self.current_language, 'scope_audio'))
        self.case_label.setText(get_translation(self.current_language, 'case_sensitive_search'))

    def setup_connections(self):
        self.cb_orig.toggled.connect(lambda v: self._update_flag('search_scope_original', v))
        self.cb_ref.toggled.connect(lambda v: self._update_flag('search_scope_reference', v))
        self.cb_edit.toggled.connect(lambda v: self._update_flag('search_scope_editor', v))
        self.cb_audio.toggled.connect(lambda v: self._update_flag('search_scope_audio', v))
        self.case_toggle.toggled.connect(lambda v: self._update_flag('search_case_sensitive', v))

    def _update_flag(self, attr_name, value):
        if self.parent_window:
            setattr(self.parent_window, attr_name, value)
            # Re-run search if there is text
            if hasattr(self.parent_window, 'search_input') and self.parent_window.search_input.text():
                self.parent_window.on_search_text_changed(self.parent_window.search_input.text())

    def update_from_parent(self):
        """Синхронизирует состояние контролов с флагами родителя"""
        if self.parent_window:
            self.cb_orig.setChecked(getattr(self.parent_window, 'search_scope_original', True))
            self.cb_ref.setChecked(getattr(self.parent_window, 'search_scope_reference', True))
            self.cb_edit.setChecked(getattr(self.parent_window, 'search_scope_editor', True))
            self.cb_audio.setChecked(getattr(self.parent_window, 'search_scope_audio', True))
            self.case_toggle.setChecked(getattr(self.parent_window, 'search_case_sensitive', False))

    def hideEvent(self, event):
        """Уведомляет родителя о закрытии окна"""
        if self.parent_window and hasattr(self.parent_window, 'on_search_popup_hidden'):
            self.parent_window.on_search_popup_hidden()
