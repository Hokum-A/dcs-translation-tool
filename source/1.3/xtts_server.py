...
def start_watchdog():
    """Следит за смертью родителя и убивает сервер (работает в т.ч. на Windows)."""
    import threading
    import time
    import os
    ...
    def watchdog():
        if sys.platform == "win32":
            try:
                ...
                if handle:
                    ctypes.windll.kernel32.WaitForSingleObject(handle, 0xFFFFFFFF) # Ждет завершения родителя
        ...
        while True:
            time.sleep(3)
            try:
                if os.getppid() != parent_pid and sys.platform != "win32":
                    os._exit(0)
            except Exception:
                os._exit(0)
