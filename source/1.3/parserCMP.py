...
class CampaignParser:
    ...
    def _process_line(self, line: str):
        ...
        # Ищем начало ключа: ["key"] = или просто key =
        # Добавляем захват отступа
        key_match = re.search(r'^(\s*)(?:\["([^"]+)"\]|([a-zA-Z0-9_]+))\s*=\s*', line)
        if key_match:
            if self.current_key:
                self._save_current_entry()
            ...
            self.current_key = key_match.group(2) or key_match.group(3)
            self.current_raw_parts = []
    ...
    def _prepare_display_data(self) -> List[Dict]:
        ...
        for entry in self.entries:
            ...
            if base in self.base_keys:
                ...
                # Строгое соответствие минимальному отступу для исключения вложенных ключей
                if entry['indent'] == min_indent:
        ...
        # Сначала base_keys в заданном порядке. 
        # Мы берем только те локали, которые реально присутствуют в файле для этого ключа.
        for b_key in self.base_keys:
            if b_key not in root_entries:
                continue
                
            # Получаем существующие языки для этого ключа (если есть)
            existing_langs = sorted(list(root_entries[b_key].keys()), key=lambda x: (x != 'DEFAULT', x != 'EN', x))
        ...
        for b_key in sorted(other_root_entries.keys()):
             for lang in sorted(other_root_entries[b_key].keys(), key=lambda x: (x != 'DEFAULT', x)):
